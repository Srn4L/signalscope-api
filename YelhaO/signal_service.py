...
def compute_need_score(
    ...
    """
    Estimate how much a business may need help — independent of opportunity_score.

    High need = strong digital gaps, weak infrastructure, reachable business.
    Does NOT conflate need with opportunity or quality.
    ...
    if not candidate:
        candidate = {}
    if not input_intelligence:
        input_intelligence = {}

    score    = 0
    reasons: list[str] = []
    confidence_hits = 0  # signals that raise confidence
    confidence_misses = 0
    ...
    v_status = (candidate.get("verification_status") or "").lower()

    has_ig   = bool(candidate.get("instagram_url") or candidate.get("instagram"))
    has_fb   = bool(candidate.get("facebook_url")  or candidate.get("facebook"))
    raw_data = candidate.get("raw_data") or {}
    has_booking = bool(
        raw_data.get("booking_url") or
        any(kw in website.lower() for kw in ("booksy", "fresha", "vagaro", "mindbody", "calendly"))
    )

    # -- Website signals -------------------------------------------------------
    if not website:
        score += 25
        reasons.append("No website confirmed")
        confidence_misses += 1
    elif v_status in ("unknown", "unverified"):
        score += 12
        reasons.append("Website not yet verified")
        confidence_misses += 1
    else:
        confidence_hits += 1
    ...
    if not has_ig and not has_fb:
        score += 18
        reasons.append("No social profiles detected")
        confidence_misses += 1
    elif has_ig or has_fb:
        confidence_hits += 1

    # -- Booking signals -------------------------------------------------------
    if not has_booking:
        score += 18
        reasons.append("Booking path not clearly surfaced")

    # -- Review signals --------------------------------------------------------
    if reviews == 0:
        score += 10
        reasons.append("No reviews found — limited social proof")
        confidence_misses += 1
    elif reviews < 20:
        score += 7
        reasons.append(f"Low review count ({reviews}) for category")
    elif reviews < 5 and rating >= 4.0:
        score += 5
        reasons.append("High rating but very few reviews — growth potential")

    if 0 < rating < 3.5 and reviews > 10:
        score += 10
        reasons.append("Below-average rating with real customer volume")

    # -- Source signals --------------------------------------------------------
    if source in ("social_indexed", "booking_indexed"):
        score += 8
        reasons.append("Discovered via non-Google source — lower saturation")
        confidence_misses += 1  # inferred source, less verified data

    if v_status in ("blocked", "unknown"):
        # Blocked source → reduce confidence, not score
        confidence_misses += 1

    # -- Input intelligence adjustments ----------------------------------------
    problem_signal = (input_intelligence.get("problem_signal") or "").lower()
    if "social" in problem_signal and not has_ig:
        score += 8
        reasons.append("Matches queried problem: weak social presence")
    if "booking" in problem_signal and not has_booking:
        score += 8
        reasons.append("Matches queried problem: missing booking path")
    if "website" in problem_signal and not website:
        score += 8
        reasons.append("Matches queried problem: no website")

    score = _clamp(score, 0, 100)

    # Level
    if score >= 65:
        level, label = "high", "High Need"
    elif score >= 35:
        level, label = "medium", "Moderate Need"
    else:
        level, label = "low", "Low Need"

    # Confidence
    total_signals = confidence_hits + confidence_misses
    if total_signals == 0:
        conf = "low"
    elif confidence_misses > confidence_hits:
        conf = "low"
    elif confidence_hits >= 2:
        conf = "medium" if confidence_misses > 0 else "high"
    else:
        conf = "medium"

    # Plain English summary
    if score >= 65:
        plain = "This business appears active but its digital path from discovery to booking is unclear or weak."
    elif score >= 35:
        plain = "This business has some digital presence but visible gaps that could be addressed."
    else:
        plain = "This business has solid digital infrastructure — need may be lower but opportunities can still exist."

    print(
        f"[NeedScore] {candidate.get('business_name','?')}: level={level} score={score}",
        flush=True,
    )
...
def assign_opportunity_lane(
    candidate: dict,
    need_score: dict | None = None,
    input_intelligence: dict | None = None,
) -> str:
    """
    Assign one of four opportunity lanes to a candidate.

    Lanes:
        proven_winner   — Strong demand, high opp score, low/medium need.
        hidden_gap      — Decent demand, clear gap, medium/high need.
        help_needed     — High need, weaker digital, reachable.
        exploration_pick — Low saturation, unusual source, uncertain.

    Args:
        candidate:          Business dict with opportunity_score, source, etc.
        need_score:         Output of compute_need_score().
        input_intelligence: Parsed intent context.

    Returns:
        Lane string.
    """
    opp_score  = int(candidate.get("opportunity_score") or 0)
    source     = (candidate.get("source") or "").lower()
    ns         = need_score or {}
    need_level = ns.get("level", "low")
    need_val   = int(ns.get("score") or 0)

    lane: str

    if opp_score >= 70 and need_level == "low":
        lane = "proven_winner"

    elif opp_score >= 55 and need_level in ("medium", "high"):
        lane = "hidden_gap"

    elif need_level == "high":
        lane = "help_needed"

    elif source in ("social_indexed", "booking_indexed") or opp_score < 45:
        lane = "exploration_pick"
    ...
    print(f"[Lane] {candidate.get('business_name','?')}: {lane}", flush=True)
    return lane
...
def build_freshness_signal(
    candidate: dict,
    source_metadata: dict | None = None,
) -> dict:
    """
    Build a lightweight freshness signal for a Scout candidate.

    Freshness reflects whether the signal data is current, stale, or unknown.
    It does NOT override opportunity fundamentals — it modulates confidence.

    Args:
        candidate:        Business dict.
        source_metadata:  Optional dict with "discovered_at", "source_age_days".
    ...
    """
    if not candidate:
        return {"level": "unknown", "last_verified_at": "", "source_age_days": None, "reason": "No candidate data."}

    sm       = source_metadata or {}
    source   = (candidate.get("source") or "").lower()
    v_status = (candidate.get("verification_status") or "").lower()

    age_days: int | None = sm.get("source_age_days")

    # Blocked or unknown verification → unknown freshness
    if v_status in ("blocked", "unknown"):
        return {
            "level":            "unknown",
            "last_verified_at": "",
            "source_age_days":  age_days,
            "reason":           "Source was blocked or unverified — freshness unknown.",
        }

    # Newly discovered in current Scout run
    if sm.get("discovered_now") or source in ("google_text", "google_nearby", "social_indexed", "booking_indexed"):
        level  = "fresh"
        reason = "Discovered in this Scout session — signals reflect current state."
        print(f"[Freshness] {candidate.get('business_name','?')}: level=fresh", flush=True)
        return {
            "level":            level,
            "last_verified_at": sm.get("discovered_at", ""),
            "source_age_days":  0,
            "reason":           reason,
        }

    # Geoapify or booking fallback
    if source in ("geoapify", "booking_platform"):
        level  = "recent"
        reason = "Sourced from directory data — typically updated regularly."
        print(f"[Freshness] {candidate.get('business_name','?')}: level=recent", flush=True)
        return {
            "level":            level,
            "last_verified_at": sm.get("discovered_at", ""),
            "source_age_days":  age_days,
    ...
    # DB-cached result with no recent verification
    if age_days and age_days > 30:
        level  = "stale"
        reason = f"Data is approximately {age_days} days old without re-verification."
    elif age_days and age_days > 7:
        level  = "recent"
        reason = f"Data is {age_days} days old — likely still accurate."
    else:
        level  = "unknown"
        reason = "Source age unknown — freshness cannot be determined."

    print(f"[Freshness] {candidate.get('business_name','?')}: level={level}", flush=True)
    return {
        "level":            level,
        "last_verified_at": sm.get("discovered_at", ""),
        "source_age_days":  age_days,
...
_HIGH_RISK_KEYWORDS = [
    "death", "died", "accident", "tragedy", "shooting", "violence",
    "lawsuit", "arrest", "scandal", "fraud", "abuse", "assault",
    "crisis", "emergency", "disaster", "health scare",
]

_EVENT_TYPE_MAP = [
    (["viral", "trending", "tiktok trend", "instagram trend", "going viral"],            "trend"),
    (["complaint", "bad review", "negative review", "disappointed", "rip off"],         "consumer_complaint"),
    (["controversy", "backlash", "called out", "canceled"],                             "controversy"),
    (["competitor launched", "new location", "grand opening", "expansion"],             "competitor_move"),
    (["seasonal", "holiday", "summer", "winter", "back to school", "valentines"],       "seasonal"),
    (["local event", "festival", "community", "pop-up", "fair", "market"],             "local_event"),
    (["pricing", "price hike", "too expensive", "cheap option", "affordable"],         "consumer_complaint"),
    (["trust", "authentic", "real", "genuine", "transparent", "behind the scenes"],    "trend"),
]
...
def build_event_intelligence(
    ...
    search_snippets: list | None = None,
) -> dict:
    """
    Detect relevant events, trends, discourse, and seasonal moments that
    create timely conversion opportunities for a business.

    Core question: Can this event help this user create value for this business?
    Do NOT recommend exploiting harmful or high-risk events.

    Args:
        query_context:    Dict with niche, location, service_angle, mode.
        business:         Business dict.
        signals:          Output of build_opportunity_signals().
        trend_data:       Trend intelligence dict from app.py.
        search_snippets:  List of web search result bodies.

    Returns:
        Full event intelligence dict as specified in the product brief.
    """
    if not query_context:
        query_context = {}

    niche          = (query_context.get("niche") or (business or {}).get("category") or "").lower()
    service_angle  = (query_context.get("service_angle") or "").lower()
    mode           = (query_context.get("mode") or "outreach").lower()

    # Gather all text to scan
    scan_parts: list[str] = []
    if trend_data:
        for v in trend_data.values():
            if isinstance(v, list):
                scan_parts += [str(x) for x in v]
            elif isinstance(v, str):
                scan_parts.append(v)
    if search_snippets:
        scan_parts += [str(s) for s in search_snippets]

    haystack = " ".join(scan_parts).lower()

    # Detect event type
    event_type = "none"
    for keywords, etype in _EVENT_TYPE_MAP:
    ...
    for kw in _HIGH_RISK_KEYWORDS:
        if kw in haystack:
            risk_level = "high"
            do_not_use = (
                f"Potential high-risk content detected ('{kw}'). "
                "Avoid exploiting sensitive events. Focus on the underlying customer concern instead."
            )
            break
    ...
    if event_type != "none":
        relevance_score += 30
        relevance_reasons.append(f"Detected event type: {event_type}")

    if niche and niche in haystack:
        relevance_score += 25
        relevance_reasons.append(f"Event discourse matches niche: {niche}")

    if service_angle and any(sa in haystack for sa in service_angle.split("_")):
        relevance_score += 20
        relevance_reasons.append(f"Event relates to service angle: {service_angle}")

    # High-risk events should NOT boost score
    if risk_level == "high":
        relevance_score = min(relevance_score, 20)

    relevance_score = _clamp(relevance_score, 0, 100)
    ...
    # Build content/outreach angles
    content_angles: list[str] = []
    if event_type == "trend":
        content_angles = [
            f"Create content showing how your {niche} work connects to the current trend.",
            "Show before/after or transformation content while this is top of mind.",
        ]
    elif event_type == "consumer_complaint":
        content_angles = [
            "Address the customer concern directly — transparency builds trust.",
            "Show your process, pricing clarity, or behind-the-scenes to contrast with the complaint pattern.",
        ]
    elif event_type == "seasonal":
        content_angles = [
            f"Seasonal offers or themed content matching current demand for {niche}.",
            "Promote advance booking or special packages tied to the seasonal moment.",
        ]
    elif event_type == "competitor_move":
        content_angles = [
            "Highlight what makes your business different without directly attacking competitors.",
            "Capture attention from customers exploring alternatives.",
        ]
    elif event_type == "local_event":
        content_angles = [
            "Align your outreach with local event timing to increase visibility.",
            "Offer event-adjacent specials or partner with event organizers.",
        ]
    ...
    if risk_level == "high":
        ...
        recommended_use = "content" if "content" in service_angle else "outreach"
    elif relevance_level == "medium":
        recommended_use = "monitor"
    else:
        recommended_use = "monitor"
    ...
    why_now = ""
    urgency = "low"
    if event_type == "trend" and relevance_level in ("high", "medium"):
        why_now  = f"Active trend in the {niche} category — window to act is typically 1-3 weeks."
        urgency  = "high" if relevance_level == "high" else "medium"
    elif event_type == "seasonal":
        why_now  = "Seasonal demand peak approaching — early content and outreach wins the booking."
        urgency  = "medium"
    elif event_type == "consumer_complaint":
        why_now  = "Customer sentiment is primed — addressing the concern now builds differentiation."
        urgency  = "medium"

    # Summary
    if event_type == "none" or relevance_level == "low":
        summary = "No strong event signal detected for this niche at this time."
        plain   = "No major trend or event was detected that would create an urgent timing advantage."
    else:
        summary = f"{event_type.replace('_',' ').title()} detected in the {niche} category."
        plain   = (
            f"There is a {relevance_level}-relevance {event_type.replace('_',' ')} in this niche. "
            f"{why_now or 'Consider how this affects your outreach timing.'}"
        )

    # Log
    if relevance_level != "low":
        print(
            f"[EventIntel] category={niche} event_type={event_type} "
            f"relevance={relevance_level} risk={risk_level}",
            flush=True,
        )
        if why_now:
            print(f"[EventIntel] why_now={why_now[:80]}", flush=True)
    ...
    return {
        "event_relevance": {
            ...
            "summary":          summary,
            "why_it_matters":   " | ".join(relevance_reasons) if relevance_reasons else "",
            "customer_concern": "",
            "conversion_angle": content_angles[0] if content_angles else "",
        ...
        "content_or_outreach_angles": content_angles,
        "do_not_use_warning":         do_not_use,
    }
