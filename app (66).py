...
def run_deep_job(job_id, business_name, location, website_pages, social_text,
                 ...
                 objective_mode=None, discovery_metadata=None):
    """
    ...
    discovery_metadata = identity-led or social-led discovery results (rating, reviews, confidence, etc.)
    """
    try:
        ...
        # Generate verdict (decision-forward summary)
        report["verdict"] = generate_verdict(report, objective_mode)
        ...
        # Attach full growth metrics + confidence to report
        dq   = agent_brain.get("data_quality", {})
        conf = confidence_from_data_quality(dq)
        niche_hint = report.get("industry", "") or (discovery_metadata or {}).get("niche", "") or ""
        growth_metrics = estimate_growth_metrics(report, niche_hint)
        report["growth_metrics"]     = growth_metrics
        report["metrics_confidence"] = growth_metrics.get("metrics_confidence", conf)
...
def generate_verdict(report, objective_mode=None):
    """
    Generate a one-sentence verdict for the top of the Analyze report.
    Decision-forward with emoji signal (✅/⚠️/❌).
    
    Based on:
    - overall_score
    - objective_mode (if from Scout)
    - key signals from the report
    """
    score = report.get("overall_score", 0)
    
    # Default verdicts by score range
    if score >= 75:
        emoji = "✅"
    elif score >= 55:
        emoji = "⚠️"
    ...
    if objective_mode == "outreach":
        if score >= 75:
            return f"{emoji} Strong Outreach Candidate — Clear improvement opportunity with proven demand."
        elif score >= 55:
            return f"{emoji} Moderate Outreach Potential — Some digital gaps but requires careful positioning."
        else:
            return f"{emoji} Weak Outreach Fit — Limited improvement upside or unclear demand signals."
    
    elif objective_mode == "referral":
        if score >= 75:
            return f"{emoji} Excellent Referral Partner — Strong local presence and trust signals."
        elif score >= 55:
            return f"{emoji} Viable Referral Option — Active locally but verify complementary fit."
        else:
            return f"{emoji} Limited Referral Value — Weak local presence or misaligned audience."
    
    elif objective_mode == "partnership":
        if score >= 75:
            return f"{emoji} High Partnership Fit — Strong visibility and complementary positioning."
        elif score >= 55:
            return f"{emoji} Partnership Worth Exploring — Some alignment but verify mutual benefit."
        else:
            return f"{emoji} Poor Partnership Match — Limited visibility or competitive overlap."
    
    elif objective_mode == "acquisition":
        if score >= 75:
            return f"{emoji} Strong Acquisition Target — Clear operational improvement upside."
        elif score >= 55:
            return f"{emoji} Potential Acquisition — Some improvement areas but assess carefully."
        else:
            return f"{emoji} Weak Acquisition Case — Limited improvement potential or high complexity."
    
    elif objective_mode == "market":
        if score >= 75:
            return f"{emoji} Market Leader — Category-defining presence worth studying."
        elif score >= 55:
            return f"{emoji} Established Player — Relevant market reference point."
        else:
            return f"{emoji} Peripheral Player — Limited market influence."
    
    elif objective_mode == "venture":
        if score >= 75:
            return f"{emoji} High Venture Signal — Underserved niche with growth potential."
        elif score >= 55:
            return f"{emoji} Venture Possibility — Some market gap indicators worth exploring."
        else:
            return f"{emoji} Low Venture Interest — Saturated space or unclear differentiation."
    
    # Generic verdict (no objective_mode)
    if score >= 75:
        return f"{emoji} Strong Opportunity — Multiple positive signals across digital presence and market position."
    elif score >= 55:
        return f"{emoji} Mixed Signals — Some strengths but proceed with careful assessment."
    else:
        return f"{emoji} Limited Opportunity — Weak signals across key metrics."
...
def score_and_explain(candidate, mode, soft_preferences=None, exclusions=None):
    ...
    quick_insight = _generate_quick_insight(candidate, mode, signals, opportunity_score)
...
def _generate_quick_insight(candidate, mode, signals, score):
    """
    Generate one plain-English sentence explaining why this business is worth looking at.
    Based on mode and top signals only. No strategy, no outreach copy.
    """
    rating = candidate.get("rating") or 0
    ...
    # Mode-specific insight templates
    if mode == "outreach":
        if signals.get("digital_gap") and signals.get("local_presence"):
            return f"Proven local demand ({review_count} reviews), but no visible booking path — strong outreach opportunity."
        elif signals.get("digital_gap"):
            return "Weak digital infrastructure despite being active locally — clear improvement opportunity."
        elif signals.get("no_booking") and review_count > 20:
            return "Active business without online booking — you could solve their biggest bottleneck."
        elif signals.get("low_review_count"):
            return "Early-stage business with room for growth — reachable and adaptable."
        else:
            return "Shows digital gaps that create outreach entry points."
    
    elif mode == "referral":
        if signals.get("local_presence") and signals.get("mid_review_count"):
            return f"Established local presence ({rating}★, {review_count} reviews) — ideal referral network fit."
        elif signals.get("local_presence"):
            return "Trusted locally with active customer base — strong referral partner potential."
        elif signals.get("has_website") and signals.get("strong_social"):
            return "Visible and engaged business — good complementary fit for your network."
        else:
            return "Active local business with network-building potential."
    
    elif mode == "partnership":
        if signals.get("strong_social") and signals.get("has_website"):
            return "Strong online presence and audience — co-growth partnership potential."
        elif signals.get("strong_social"):
            return "Active social following — could amplify mutual reach."
        elif signals.get("mid_review_count"):
            return "Complementary niche with moderate scale — partnership-ready."
        else:
            return "Visible, active business in adjacent space — partnership candidate."
    
    elif mode == "acquisition":
        if signals.get("digital_gap") and signals.get("low_rating"):
            return "Operational gaps with clear improvement upside — acquisition candidate."
        elif signals.get("digital_gap"):
            return "Under-optimized infrastructure — room for operational value-add."
        elif signals.get("poor_seo"):
            return "Weak SEO and online presence — acquisition upside through optimization."
        else:
            return "Shows improvement potential through better operations."
    
    elif mode == "market":
        if signals.get("high_review_count"):
            return f"Category-defining business ({review_count} reviews) — key market benchmark."
        elif signals.get("strong_social"):
            return "High visibility in local market — important competitive reference point."
        else:
            return "Established player that shapes local market dynamics."
    
    elif mode == "venture":
        if signals.get("digital_gap") and signals.get("low_review_count"):
            return "Serving niche with minimal competition — potential market gap."
        elif signals.get("digital_gap"):
            return "Early-stage operator in underserved segment — venture signal."
        else:
            return "Shows innovation or underserved market potential."
    
    # Fallback
    return "Shows opportunity signals worth investigating."
...
def estimate_growth_metrics(lead, niche=''):
    ...
    # active customer base rough estimate
    monthly_customers_est = max(20, round(visits_per_year * 8))
    revenue_estimate_est  = round(avg_ticket * visits_per_year * monthly_customers_est)
    ...
    return {
        ...
        # exposed assumptions for editable frontend panel
        'avg_ticket':        avg_ticket,
        'visits_per_year':   visits_per_year,
        'retention_years':   round(effective_retention, 1),
        'monthly_customers': monthly_customers_est,
        'revenue_estimate':  revenue_estimate_est,
        'benchmark_note':    f'Based on modeled estimates using {b.get("industry_label", "US small business")} industry averages.',
        'industry_label':    b.get('industry_label', 'US small business'),
