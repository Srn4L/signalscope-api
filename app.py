...
NICHE_BENCHMARKS = {
    'nail':    {'avg_ticket': 75,  'visits_per_year': 8,  'retention_years': 2.5, 'cac_base': 22},
    'barber':  {'avg_ticket': 35,  'visits_per_year': 18, 'retention_years': 3.0, 'cac_base': 18},
    'hair':    {'avg_ticket': 110, 'visits_per_year': 6,  'retention_years': 2.5, 'cac_base': 28},
    'spa':     {'avg_ticket': 140, 'visits_per_year': 5,  'retention_years': 2.0, 'cac_base': 35},
    'med spa': {'avg_ticket': 300, 'visits_per_year': 4,  'retention_years': 2.0, 'cac_base': 55},
    'gym':     {'avg_ticket': 60,  'visits_per_year': 12, 'retention_years': 1.5, 'cac_base': 40},
    'default': {'avg_ticket': 90,  'visits_per_year': 7,  'retention_years': 2.0, 'cac_base': 30},
}
...
def estimate_growth_metrics(lead, niche=''):
    ...
    # Churn proxy: weak digital signals = higher churn risk
    score = lead.get('overall_score', 50)
    platform = (lead.get('platform') or '').lower()

    churn_rate = 0.35  # baseline 35% annual churn
    if score < 35:
        churn_rate = 0.55   # poor signals = high churn
    elif score < 55:
        churn_rate = 0.42
    else:
        churn_rate = 0.28   # stronger presence = better retention
    ...
    return {
        ...
        'churn_proxy':      round(churn_rate * 100),   # as %
        'benchmark_note':   'Based on modeled estimates using US beauty & wellness industry averages (avg ticket size, visit frequency, and retention patterns).',
    }
    ...
    for biz in results[:limit]:
        try:
            ...
            growth_metrics = estimate_growth_metrics(
                {"overall_score": report.get("overall_score", 50), "platform": biz["platform"]},
                niche=niche
            )
