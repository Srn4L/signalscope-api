...
def _domain_matches_business(business_name: str, url: str) -> tuple:
    """
    Check if a URL domain plausibly matches the requested business name.
    Returns (is_match: bool, reason: str).
    Fails open — returns True on any parse error so we never block valid requests.
    """
    if not url or not business_name:
        return True, "no_url_to_check"
    try:
        from urllib.parse import urlparse as _up
        domain = _up(url if url.startswith("http") else "https://" + url).netloc
        domain = domain.replace("www.", "").lower().split(":")[0]
        domain_root = domain.split(".")[0]

        # Generic/booking/social platforms always pass
        _GENERIC = {"booksy","fresha","vagaro","yelp","google","facebook","instagram",
                    "tripadvisor","yellowpages","bing","wix","squarespace","wordpress",
                    "shopify","square","mindbody","calendly","linktree","beacons"}
        if domain_root in _GENERIC or any(g in domain for g in _GENERIC):
            return True, "generic_platform"

        # Token overlap: business name tokens vs domain root
        biz_tokens = set(re.sub(r'[^a-z0-9]', ' ', business_name.lower()).split())
        biz_tokens -= {"the","a","an","of","and","or","in","at","for","by","llc","inc","co","ltd","restaurant","salon","services","service","group","house","studio"}
        dom_str = re.sub(r'[^a-z0-9]', '', domain_root)

        if not biz_tokens:
            return True, "no_tokens"

        # Direct overlap or substring match on tokens of length >= 4
        for tok in biz_tokens:
            if len(tok) >= 4 and (tok in dom_str or dom_str in tok):
                return True, f"token_match:{tok}"

        exact_overlap = biz_tokens & set(re.sub(r'[^a-z0-9]', ' ', domain_root).split())
        if exact_overlap:
            return True, f"exact_overlap:{exact_overlap}"

        return False, f"no_overlap biz_tokens={biz_tokens} domain_root={domain_root}"
    except Exception:
        return True, "parse_error_fail_open"
...
try:
    ...
    print(f"[WARN] database unavailable: {_db_import_err}", flush=True)
    _DB_AVAILABLE = False

# --- INTELLIGENCE SERVICES (Phase 6) -----------------------------------------
try:
    from signal_service import (
        ...
        compute_need_score,
        assign_opportunity_lane,
        build_freshness_signal,
...
def get_token_hash(req) -> str | None:
    raw = req.headers.get("X-Agent-Token", "").strip().lower()
    if not raw:
        return None
...
def safe_get(url, timeout=10):
    '''
    Fetch raw HTML for structural parsing: link detection, social extraction,
    platform detection, booking card parsing, schema detection.

    Priority: direct requests (free) → ScraperAPI fallback (only when blocked).
    Never uses Firecrawl — raw HTML is required for href/meta/script parsing.

    Return: (html_text | None, status_code | error_str).
    Side-effect: writes to FETCH_CLASSIFICATIONS[url].

    For AI prompt text extraction, use fetch_for_analysis() instead.
    '''
    # -- Direct request (free, no credits burned) ----------------------------
    try:
        r = req.get(url, headers=HEADERS, timeout=10)
        r.raise_for_status()
        FETCH_CLASSIFICATIONS[url] = {"source_status": "ok", "asset_state": "found", "provider": "direct"}
        ...
        _blocked = _cls.get("source_status") in ("blocked", "timeout")
        if not _blocked:
            # Non-block failure (DNS, SSL, redirect) — ScraperAPI won't help
            log_error("Direct fetch", e, url=url)
            return None, str(e)
        print(f"[Fetch] direct blocked — trying ScraperAPI: {url}", flush=True)

    # -- ScraperAPI fallback (Cloudflare/JS-rendered/geo-blocked pages) -------
    if SCRAPER_API_KEY:
        try:
            proxy_url = (
            ...
            r.raise_for_status()
            FETCH_CLASSIFICATIONS[url] = {"source_status": "ok", "asset_state": "found", "provider": "scraperapi"}
            return r.text, r.status_code
        except Exception as e:
...
def fetch_for_analysis(url, timeout=15) -> str:
    '''
    Fetch clean text content for AI prompt injection.
    Priority: Firecrawl (clean markdown) → ScraperAPI + extract_text → direct + extract_text.

    Returns plain text string (never None, never raw HTML).
    Use this for website_pages content passed to GPT/Claude.
    Use safe_get() for anything needing raw HTML structure.
    '''
    # -- Firecrawl (clean markdown — best for LLM pipelines) -----------------
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if md and len(md) > 100:
                    print(f"[Fetch] fetch_for_analysis provider=firecrawl url={url[:60]}", flush=True)
                    return md[:4000]
            elif r.status_code in (402, 429):
                print(f"[Fetch] Firecrawl quota/rate ({r.status_code}) — falling back", flush=True)
            else:
                print(f"[Fetch] Firecrawl HTTP {r.status_code} — falling back", flush=True)
        except Exception as e:
            log_error("Firecrawl fetch_for_analysis", e, url=url)

    # -- ScraperAPI → extract_text fallback ----------------------------------
    html, _ = safe_get(url, timeout=timeout)
    if html:
...
def search_web(query, max_results=6):
    '''
    Discovery and operator search. Priority: SerpAPI → DuckDuckGo.
    Use for: business discovery, site: operators, booking platforms,
             social-indexed queries, official website lookup.

    For review context, press, market mentions — use search_context() instead.
    '''
    # -- SerpAPI (Google) — primary for discovery/operators -------------------
    if SERP_API_KEY:
        try:
            ...
            if r.status_code == 200:
                ...
                if snippets:
                    return snippets
                # SerpAPI returned 200 but no organic results (quota/plan limit)
                print(f"[Search] SerpAPI 200 but 0 results for query={query!r:.60} — falling back to DDG", flush=True)
            else:
                print(f"[Search] SerpAPI HTTP {r.status_code} — falling back to DDG", flush=True)
        except Exception as e:
            log_error("SerpAPI search", e, fallback="DuckDuckGo")
    else:
        print(f"[Search] SERP_API_KEY not set — using DDG only", flush=True)
...
def search_context(query, max_results=6):
    '''
    Context and enrichment search. Priority: Tavily → SerpAPI → DuckDuckGo.
    Use for: reviews, press mentions, market context, pricing research,
             reputation analysis, competitor summaries.

    Tavily is optimized for AI context queries and returns richer snippets
    for natural language queries. SerpAPI is better for operator/local queries,
    so use search_web() for those instead.
    '''
    # -- Tavily (AI-optimized — primary for context queries) ------------------
    if TAVILY_API_KEY:
        try:
            r = req.post(
            ...
            if r.status_code == 200:
                snippets = []
                for result in (r.json().get("results") or [])[:max_results]:
                    snippets.append({
                        "title":    result.get("title", ""),
                        "url":      result.get("url", ""),
                ...
                if snippets:
                    return snippets
                print(f"[Search] search_context Tavily 0 results — falling back", flush=True)
            elif r.status_code in (401, 403):
                print(f"[Search] Tavily auth error {r.status_code} — check TAVILY_API_KEY", flush=True)
            else:
                print(f"[Search] Tavily HTTP {r.status_code} — falling back", flush=True)
        except Exception as e:
            log_error("Tavily search_context", e, fallback="SerpAPI")
...
def discover_business_signals(business_name, location, industry=""):
    ...
    try:
        results = search_context(f'"{business_name}" {location} reviews', 3)
...
def run_fast_pipeline(business_name, location, website_pages, social_text, social_links=None, gp_context=None):
    '''
    ...
    gp_context: optional dict with google_rating, google_review_count, discovered_website
                from discover_business_signals(). Injected into prompt so fast score
                reflects real-world reputation, not just scraped content availability.
    '''
    import textwrap
    raw = {
    ...
    # Google Places context — inject real reputation signals so fast score isn't
    # artificially low just because scraping returned thin content
    gp_note = ""
    gp = gp_context or {}
    gp_rating  = float(gp.get("rating") or gp.get("google_rating") or 0)
    gp_reviews = int(gp.get("review_count") or gp.get("google_review_count") or 0)
    if gp_rating and gp_reviews:
        gp_note = (
            f"\nVERIFIED REPUTATION DATA (Google Places — treat as high confidence):"
            f"\n  Rating: {gp_rating} stars from {gp_reviews} reviews"
            f"\n  IMPORTANT: A business with {gp_reviews}+ reviews has PROVEN LOCAL DEMAND."
            f"\n  The overall_score MUST reflect this — do NOT score below 55 solely because"
            f"\n  social content was unavailable. High review count + good rating = opportunity,"
            f"\n  not a weak signal. Score the OUTREACH OPPORTUNITY, not just content quality."
        )
    ...
    {platform_hint}{social_presence_note}{gp_note}
    ...
    - overall_score = outreach/opportunity score (0-100), NOT social content quality score
    - A business with 100+ reviews proving local demand should score 55+ even with thin social
    {{
...
def run_deep_job(job_id, business_name, location, website_pages, social_text,
    ...
    try:
        ...
        # Verdict — mode-aware, score-tiered decision signal
        _verdict_mode = objective_mode or mode or "outreach"
        _verdict       = generate_verdict(report, _verdict_mode)
        report["verdict_emoji"] = _verdict["emoji"]
        report["verdict_text"]  = _verdict["text"]
        report["verdict_tier"]  = _verdict["tier"]
...
def agent():
    ...
    if analysis_mode == "identity_led":
        ...
        # Source check — validate discovered website matches the business
        if website_url:
            _sm, _sr = _domain_matches_business(business_name, website_url)
            print(f"[SourceCheck] requested={business_name!r} domain={website_url!r} status={'match' if _sm else 'mismatch'}", flush=True)
            if not _sm:
                print(f"[SourceCheck] MISMATCH — discarding {website_url!r} for {business_name!r} | reason={_sr}", flush=True)
                website_url = ""
                discovery_metadata["data_quality_warning"] = "Discovered website domain does not match business name — discarded to prevent data contamination."
                discovery_metadata["source_confidence"]    = "low"

    # -- Source validation for user-provided website -------------------------
    _source_mismatch_warning = None
    if website_url and business_name:
        _sm2, _sr2 = _domain_matches_business(business_name, website_url)
        print(f"[SourceCheck] provided url={website_url!r} business={business_name!r} status={'match' if _sm2 else 'mismatch'}", flush=True)
        if not _sm2:
            _source_mismatch_warning = "Provided website domain does not closely match business name — analysis may reflect different business."
            print(f"[SourceCheck] WARNING: {_source_mismatch_warning}", flush=True)
    ...
    if website_url:
        ...
        # Raw HTML needed for link/platform detection
        ...
        if html:
            ...
            for k, v in detected.items():
                if k not in social_links:
                    social_links[k] = v
        # Clean text for AI prompt — Firecrawl gives better quality here
        homepage_text = fetch_for_analysis(website_url)
        if homepage_text:
            website_pages["homepage"] = homepage_text[:1500]
        elif html:
            website_pages["homepage"] = extract_text(html, 1500)

        # Subpages — text only, Firecrawl if available
        for slug in ["/about"]:
            sub_text = fetch_for_analysis(website_url.rstrip("/") + slug)
            if sub_text:
                website_pages[slug.lstrip("/")] = sub_text[:1500]
            else:
                ...
                if html:
                    ...
                    for k, v in detected.items():
                        if k not in social_links:
                            social_links[k] = v
                # Clean text for AI
                page_text = fetch_for_analysis(u) or (extract_text(html, 1500) if html else "")
                if page_text:
                    website_pages["homepage"] = page_text[:1500]
                break
    ...
    try:
        fast_report = run_fast_pipeline(business_name, location, website_pages, social_text, social_links, gp_context=discovery_metadata)
    ...
    def deep_work():
        try:
            ...
            # Parallelize press + reviews + pricing — use search_context (Tavily-first)
            # for natural language enrichment queries, not discovery operators
            from concurrent.futures import ThreadPoolExecutor as _TPE
            def _search_ctx(q): return search_context(q, 3)
            with _TPE(max_workers=3) as ex:
                f_press   = ex.submit(_search_ctx, f'"{business_name}" {location}')
                f_reviews = ex.submit(_search_ctx, f'"{business_name}" {location} reviews')
                f_pricing = ex.submit(_search_ctx, f'{business_name} pricing cost {industry}')
            ...
            # Try Google reviews snippet — search_context (Tavily handles these well)
            google_rev = search_context(f"{business_name} {location} google reviews rating", 2)
            for r in google_rev:
                if r.get('body') and len(r['body']) > 80:
                    rev_texts.append(f"[Google Reviews]: {r['body']}")

            # Try Yelp — site: operator, use search_web (SerpAPI handles site: better)
            ...
            # Try Tripadvisor — site: operator, use search_web
            ...
            # Try Facebook reviews — natural language, use search_context
            fb_snip = search_context(f"{business_name} {location} facebook reviews", 1)
            for r in fb_snip:
    ...
    # -- Intelligence layer injection (Phase 6) --------------------------------
    # Wraps in try/except so Analyze NEVER fails if this layer errors.
    if _INTEL_AVAILABLE:
        try:
            _biz_payload = {
                "name":         business_name,
                ...
                "instagram_url": manual_socials.get("instagram"),
                "facebook_url":  manual_socials.get("facebook"),
                "raw_data":     {},
            }
            _ctx_payload = {
                "mode":              objective_mode or body.get("mode") or "outreach",
                "user_role":         body.get("user_role"),
                "signal_preferences": body.get("signal_preferences") or [],
                "niche":             industry,
...
def score_and_explain(candidate: dict, mode: str, soft_preferences=None, exclusions=None) -> dict:
    ...
    quick_insight = _generate_quick_insight(candidate, mode)
    ...
    return {
        ...
        "quick_insight":        quick_insight,       # one-line Scout card insight
...
# ─────────────────────────────────────────────────────────────────────────────
# VERDICT + QUICK INSIGHT  (Phase: Analyze verdict + Scout insight)
# ─────────────────────────────────────────────────────────────────────────────

_VERDICT_MAP = {
    # (mode, tier) -> (emoji, text)
    # Tiers: high=score>=70, medium=45-69, low=<45
    ("outreach",    "high"):   ("✅", "Strong Outreach Candidate — clear demand gap and reachable digital presence."),
    ("outreach",    "medium"): ("⚠️", "Moderate Outreach Opportunity — signals present but digital gaps are partial."),
    ("outreach",    "low"):    ("❌", "Weak Outreach Signal — limited demand or infrastructure gaps are unclear."),

    ("referral",    "high"):   ("✅", "Strong Referral Partner — trusted local presence and solid reputation signals."),
    ("referral",    "medium"): ("⚠️", "Potential Referral Partner — decent standing but limited signal depth."),
    ("referral",    "low"):    ("❌", "Weak Referral Fit — low trust signals or insufficient review presence."),

    ("partnership", "high"):   ("✅", "Strong Partnership Candidate — complementary signals and reachable scale."),
    ("partnership", "medium"): ("⚠️", "Possible Partnership — some alignment but signals are mixed."),
    ("partnership", "low"):    ("❌", "Weak Partnership Signal — insufficient scale or alignment indicators."),

    ("acquisition", "high"):   ("✅", "Acquisition Target — fixable operational gaps and undervalued signals."),
    ("acquisition", "medium"): ("⚠️", "Possible Acquisition — some upside but valuation signals are uncertain."),
    ("acquisition", "low"):    ("❌", "Low Acquisition Fit — gaps may be structural rather than fixable."),

    ("market",      "high"):   ("✅", "Strong Market Signal — high-activity business with clear positioning."),
    ("market",      "medium"): ("⚠️", "Mixed Market Signal — active but positioning or differentiation is unclear."),
    ("market",      "low"):    ("❌", "Weak Market Signal — limited data or low market presence detected."),

    ("venture",     "high"):   ("✅", "Venture Candidate — strong growth indicators and scaling signals present."),
    ("venture",     "medium"): ("⚠️", "Watch List — some growth signals but early or unverified."),
    ("venture",     "low"):    ("❌", "Not Venture-Ready — insufficient scaling or innovation signals."),
}

_QUICK_INSIGHT_MAP = {
    # (mode, has_website, has_social, has_booking) -> insight string
    ("outreach", True,  False, False): "Has a website but no social presence — content gap is the angle.",
    ("outreach", True,  True,  False): "Active online but no booking path — conversion is the gap.",
    ("outreach", True,  True,  True):  "Strong digital presence — look for SEO or retention opportunities.",
    ("outreach", False, True,  False): "Social-first business with no website — digital conversion gap.",
    ("outreach", False, False, False): "Minimal digital footprint — high outreach potential, approach carefully.",
    ("outreach", False, True,  True):  "Booking active but no website — local SEO is the angle.",
    ("outreach", True,  False, True):  "Website and booking present — social content is the gap.",
    ("partnership", True, True, True): "Established digital presence — strong partnership candidate.",
    ("partnership", True, True, False):"Active online — good partnership fit, explore content collaboration.",
    ("referral", True, True, True):    "Strong community signals — high referral network value.",
    ("referral", True, False, False):  "Web presence found but limited social — referral value is moderate.",
}

_QUICK_INSIGHT_MODE_FALLBACK = {
    "outreach":    "Signals detected — review gaps before outreach.",
    "referral":    "Decent presence — evaluate fit for referral network.",
    "partnership": "Some signals present — assess complementary fit.",
    "acquisition": "Review operational signals before pursuing.",
    "market":      "Market signals captured — compare against competitors.",
    "venture":     "Early signals only — monitor for growth indicators.",
}


def generate_verdict(report: dict, mode: str | None = None) -> dict:
    """
    Generate a mode-aware verdict from an Analyze report.

    Returns {"emoji": str, "text": str, "tier": str}
    """
    score = int(report.get("overall_score") or 0)
    if not mode:
        mode = (report.get("objective_mode") or report.get("mode") or "outreach").lower()
    if mode not in ("outreach","referral","partnership","acquisition","market","venture"):
        mode = "outreach"
    ...
    emoji, text = _VERDICT_MAP.get((mode, tier), ("⚠️", "Analysis complete — review signals below."))
    return {"emoji": emoji, "text": text, "tier": tier, "mode": mode, "score": score}


def _generate_quick_insight(candidate: dict, mode: str) -> str:
    """
    Generate a one-line Scout insight for a candidate based on its digital signals.
    Used by score_and_explain() to add quick_insight to each Scout card.
    """
    if not candidate:
        return ""

    has_website = bool((candidate.get("website") or "").strip())
    has_social  = any(candidate.get(k) for k in ("instagram_url","instagram","facebook_url","facebook","tiktok"))
    raw_data    = candidate.get("raw_data") or {}
    has_booking = bool(
        raw_data.get("booking_url") or
        any(kw in (candidate.get("website") or "").lower()
            for kw in ("booksy","fresha","vagaro","mindbody","calendly"))
    )

    mode_clean = (mode or "outreach").lower()
    key = (mode_clean, has_website, has_social, has_booking)
    if key in _QUICK_INSIGHT_MAP:
        return _QUICK_INSIGHT_MAP[key]

    # Rating-adjusted fallback
    rating  = float(candidate.get("rating") or candidate.get("google_rating") or 0)
    reviews = int(candidate.get("review_count") or candidate.get("google_review_count") or 0)
    if rating >= 4.5 and reviews >= 50:
        return "Proven local demand — strong reputation with clear digital opportunity."
    if reviews < 10 and has_website:
        return "Website present but limited social proof — trust signals are the gap."

    return _QUICK_INSIGHT_MODE_FALLBACK.get(mode_clean, "Review signals to identify the best angle.")
...
def prospect():
    ...
    # ── Refresh fields (Parts 1, 6) ───────────────────────────────────────────
    is_refresh           = bool(body.get('refresh'))
    refresh_cursor       = (body.get('refresh_cursor') or '').strip() or None
    exclude_biz_ids      = set(body.get('exclude_business_ids') or [])
    seen_names_raw       = [str(s).lower() for s in (body.get('seen_names') or [])]
    exclude_names_raw    = [str(s).lower() for s in (body.get('exclude_names') or [])]
    seen_name_set        = set(seen_names_raw + exclude_names_raw)
    source_strategy_req  = (body.get('source_strategy') or 'auto').lower()
    ...
    # ── Refresh: source rotation + exclusion suppression (Parts 1, 2, 3) ─────
    refresh_strategy_data = {"strategy": "google_standard", "next_cursor": "refresh_1", "description": "Standard Google Places discovery."}
    refresh_excluded_count = 0

    if is_refresh and _INTEL_AVAILABLE:
        try:
            refresh_strategy_data = choose_refresh_source_strategy(
                refresh_cursor       = refresh_cursor,
                input_intelligence   = input_intelligence,
                prior_results        = list(seen_name_set),
            )
            strategy = refresh_strategy_data["strategy"]
            print(f"[ScoutRefresh] active=true strategy={strategy} cursor={refresh_cursor}", flush=True)
            print(f"[ScoutRefresh] excluding {len(seen_name_set)} already shown businesses", flush=True)

            # Apply seen_name exclusions BEFORE secondary discovery
            pre_len = len(deduped)
            deduped = [
                b for b in deduped
                if b.get("business_name", "").lower() not in seen_name_set
                and re.sub(r'[^a-z0-9]', '', (b.get("business_name") or "").lower())[:20] not in seen_name_set
            ]
            refresh_excluded_count += pre_len - len(deduped)

            # Secondary source discovery
            _qctx = {"niche": niche, "location": location, "mode": mode,
                     "service_angle": input_intelligence.get("service_angle"),
                     "business_category": niche}

            if strategy in ("social_indexed", "mixed_low_saturation") or source_strategy_req in ("social", "mixed"):
                _social_candidates = discover_social_indexed_businesses(_qctx, search_fn=search_web, limit=10)
                _social_filtered = [c for c in _social_candidates if c.get("business_name","").lower() not in seen_name_set]
                deduped = deduped + _social_filtered
                print(f"[ScoutRefresh] social-indexed injected: {len(_social_filtered)}", flush=True)

            if strategy in ("booking_platforms", "mixed_low_saturation") or source_strategy_req in ("booking", "mixed"):
                _booking_candidates = discover_booking_indexed_businesses(_qctx, search_fn=search_web, limit=8)
                _booking_filtered = [c for c in _booking_candidates if c.get("business_name","").lower() not in seen_name_set]
                deduped = deduped + _booking_filtered
                print(f"[ScoutRefresh] booking-indexed injected: {len(_booking_filtered)}", flush=True)

            if strategy == "adjacent_categories":
                adj_cats = expand_adjacent_categories(niche)
                for adj in adj_cats[:2]:
                    _adj_raw = search_google_places(adj, location, limit=6)
                    for r in _adj_raw:
                        r["source"] = "adjacent_category"
                        r["discovery_reason"] = "adjacent_category_refresh"
                        if r.get("business_name","").lower() not in seen_name_set:
                            deduped.append(r)

            # Stronger exposure penalty: -30 for seen in session
            for b in deduped:
                bname = (b.get("business_name") or "").lower()
                _key  = re.sub(r'[^a-z0-9]', '', bname)[:20]
                if bname in seen_name_set or _key in seen_name_set:
                    b["_refresh_penalized"] = True
                    b["opportunity_score"]  = max(0, (b.get("opportunity_score") or 0) - 30)

            # Dedup again after injection
            seen_keys2 = set()
            deduped2   = []
            for b in deduped:
                k = re.sub(r'[^a-z0-9]', '', (b.get("business_name") or "").lower())[:20]
                if k and k not in seen_keys2:
                    seen_keys2.add(k)
                    deduped2.append(b)
            deduped = deduped2

            print(f"[ScoutRefresh] final refreshed results: {len(deduped)}", flush=True)

        except Exception as _refresh_err:
            log_error("scout refresh strategy", _refresh_err)
    ...
    # ── Part 4: Need Score + Opportunity Lane enrichment ─────────────────────
    if _INTEL_AVAILABLE:
        try:
            for b in scored:
                ns = compute_need_score(b, input_intelligence)
                b["need_score"] = ns
                b["opportunity_lane"] = assign_opportunity_lane(b, ns, input_intelligence)
                b["freshness_signal"] = build_freshness_signal(b, {"discovered_now": True})
                # Refresh card metadata
                if is_refresh:
                    b["refresh_metadata"] = {
                        "is_refresh_result":       True,
                        "source_strategy":         refresh_strategy_data.get("strategy", "google_standard"),
                        "why_new":                 "Different source and not previously shown in this Scout session",
                        "excluded_similar_count":  refresh_excluded_count,
                    }
        except Exception as _ns_err:
            log_error("need_score enrichment", _ns_err)
    ...
    if _DB_AVAILABLE:
        try:
            token_raw  = request.headers.get("X-Agent-Token", "")
            token_hash = hash_token(token_raw) if token_raw else None
    ...
    # ── Build response ────────────────────────────────────────────────────────
    results = []
    lane_counts = {"proven_winners": 0, "hidden_gaps": 0, "help_needed": 0, "exploration_picks": 0}

    for b in top_results:
        lane = b.get("opportunity_lane", "")
        if lane == "proven_winner":   lane_counts["proven_winners"]   += 1
        elif lane == "hidden_gap":    lane_counts["hidden_gaps"]       += 1
        elif lane == "help_needed":   lane_counts["help_needed"]       += 1
        elif lane == "exploration_pick": lane_counts["exploration_picks"] += 1
        ...
        card = {
            ...
            "need_score":          b.get("need_score"),
            "opportunity_lane":    lane,
            "freshness_signal":    b.get("freshness_signal"),
            "discovery_reason":    b.get("discovery_reason"),
            "verification_status": b.get("verification_status"),
            "why_now":             b.get("why_now", ""),
        }
        if b.get("refresh_metadata"):
            card["refresh_metadata"] = b["refresh_metadata"]
        results.append(card)

    # Autopilot suggestion (Part 10)
    _iq = (input_intelligence or {}).get("input_quality", "unknown")
    _autopilot = {
        "eligible":               _iq in ("medium", "strong", "high"),
        "suggested_watchlist_name": f"{niche.title()} in {location}" if niche and location else "",
        "reason":                 "Input quality is sufficient to define a repeatable watchlist.",
        "next_step_label":        "Create Watchlist",
    }
    ...
    return jsonify({
        ...
        "refresh": {
            ...
            "next_cursor":   refresh_strategy_data.get("next_cursor"),
            "description":   refresh_strategy_data.get("description"),
            "excluded_count": refresh_excluded_count,
        } if is_refresh else {"active": False, "next_cursor": refresh_strategy_data.get("next_cursor", "refresh_1")},
        "autopilot_suggestion": _autopilot,
    })
...
def list_opportunities():
    ...
    token_hash = get_token_hash(request)
    ...
    try:
        rows = get_opportunities(
            status=status, mode=mode, source=source,
...
def patch_opportunity_status(opportunity_id):
    ...
    try:
        state = update_opportunity_status(opportunity_id, body, token_hash=token_hash)
        if state is None:
...
def followups_due():
    ...
    try:
        rows = get_followups_due(token_hash=token_hash)
        results = []
        for r in rows:
            row = dict(r)
            for k, v in row.items():
...
def save_opportunity_route():
    ...
    result = save_scout_result_to_network(body, token_hash)
...
# ─────────────────────────────────────────────────────────────────────────────
# INTELLIGENCE LAYER ROUTES  (Phase 6)
# ─────────────────────────────────────────────────────────────────────────────
...
def get_business_saturation(business_id):
    ...
    if not _DB_AVAILABLE or not _INTEL_AVAILABLE:
        return jsonify({"ok": False, "error": "Intelligence services not available"}), 503

    opp_type  = request.args.get("opportunity_type")
    angle     = request.args.get("service_angle")
    target    = request.args.get("contact_target")
    method    = request.args.get("contact_method")

    try:
        saturation = get_opportunity_saturation(
        ...
        label = get_saturation_label(saturation)
        return jsonify({
            "ok":          True,
            "business_id": business_id,
        ...
        log_error("GET /businesses/<id>/saturation", e)
        return jsonify({"ok": False, "error": str(e)}), 500
...
def signals_preview():
    ...
    if not business:
        return jsonify({"ok": False, "error": "business payload is required"}), 400

    try:
        contact    = infer_best_contact_path(business, context)
        saturation = {}
        if _DB_AVAILABLE and business.get("id"):
            sat_raw = get_opportunity_saturation(
                business_id      = business["id"],
                opportunity_type = context.get("mode"),
                service_angle    = infer_service_angle(context, business),
                contact_target   = contact.get("contact_target"),
                contact_method   = contact.get("contact_method"),
            )
            saturation = sat_raw

        signals  = build_opportunity_signals(business, context, saturation, contact)
        score    = compute_opportunity_score(
            signals.get("social_conversion", {}), saturation, context
        )
...
# ═══════════════════════════════════════════════════════════════════════════
# YELHAO — CONNECTOR ARCHITECTURE + BATCH QUEUE
# Routes: /connectors, /connectors/test, /connectors/<name>/sync
#         /batch/analyze, /batch/<id>, /batch/<id>/push_crm
#         /batch/csv/upload, /batch/csv/start, /batch/<id>/export
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Batch job store (separate from deep JOBS to avoid collision) ──────────
BATCH_JOBS = {}   # batch_id -> { status, items, results, error, ts }
BATCH_TTL  = 1800  # 30 min

def _cleanup_batch_jobs():
    now = time.time()
    stale = [k for k,v in BATCH_JOBS.items()
             if v.get("status") in ("done","error") and now - v.get("ts", now) > BATCH_TTL]
    for k in stale:
        del BATCH_JOBS[k]
...
class CRMConnector(ABC):
    ...
    @abstractmethod
    def push_contact(self, data: dict) -> dict: ...
    @abstractmethod
    def push_deal(self, contact_id: str, data: dict) -> dict: ...
    @abstractmethod
    def push_note(self, contact_id: str, text: str) -> dict: ...


# ── HubSpot connector (user-supplied key) ────────────────────────────────
...
class HubSpotConnector(CRMConnector):
    ...
    def test_connection(self) -> dict:
        try:
            r = req.get(f"{self.base}/crm/v3/objects/contacts?limit=1", headers=self._h(), timeout=8)
            if r.status_code == 200:
                return {"ok": True, "connector": "hubspot"}
            return {"ok": False, "connector": "hubspot", "error": f"HTTP {r.status_code}"}
        except Exception as e:
            return {"ok": False, "connector": "hubspot", "error": str(e)}

    def push_contact(self, data: dict) -> dict:
        payload = {"properties": {
            "firstname":        data.get("business_name",""),
            "company":          data.get("business_name",""),
            "website":          data.get("website",""),
            "phone":            data.get("phone",""),
            "hs_lead_status":   "NEW",
            "yelhao_score":     str(data.get("opportunity_score","") or data.get("overall_score","")),
            "yelhao_headline":  data.get("key_insight","") or data.get("top_signal",""),
            "lifecyclestage":   "lead",
        }}
        try:
            r = req.post(f"{self.base}/crm/v3/objects/contacts", json=payload, headers=self._h(), timeout=10)
            if r.status_code in (200, 201):
                return {"ok": True, "contact_id": r.json().get("id"), "action": "created"}
            # Custom property may not exist — retry without it
            if r.status_code == 400 and "PROPERTY_DOESNT_EXIST" in r.text:
                for bad in ("yelhao_score","yelhao_headline"):
                    payload["properties"].pop(bad, None)
                r2 = req.post(f"{self.base}/crm/v3/objects/contacts", json=payload, headers=self._h(), timeout=10)
                if r2.status_code in (200, 201):
                    return {"ok": True, "contact_id": r2.json().get("id"), "action": "created_basic"}
    ...
    def push_deal(self, contact_id: str, data: dict) -> dict:
        props = {
            "dealname":  data.get("business_name","") + " — Yelhao",
            "pipeline":  "default",
            "dealstage": "appointmentscheduled",
            "amount":    "",
        }
        try:
            r = req.post(f"{self.base}/crm/v3/objects/deals",
                         json={"properties": props, "associations": [{"to": {"id": contact_id}, "types": [{"associationCategory":"HUBSPOT_DEFINED","associationTypeId":3}]}]},
                         headers=self._h(), timeout=10)
            if r.status_code in (200, 201):
                return {"ok": True, "deal_id": r.json().get("id")}
    ...
    def push_note(self, contact_id: str, text: str) -> dict:
        payload = {
            "properties": {"hs_note_body": text, "hs_timestamp": str(int(time.time()*1000))},
            "associations": [{"to": {"id": contact_id}, "types": [{"associationCategory":"HUBSPOT_DEFINED","associationTypeId":202}]}]
        }
        try:
            r = req.post(f"{self.base}/crm/v3/objects/notes", json=payload, headers=self._h(), timeout=10)
            return {"ok": r.status_code in (200,201)}
        except Exception as e:
            return {"ok": False, "error": str(e)}


# ── Salesforce connector (env-based) ─────────────────────────────────────

class SalesforceConnector(CRMConnector):
    def test_connection(self) -> dict:
        try:
            token, instance = sf_get_token()
            if token:
                return {"ok": True, "connector": "salesforce"}
            return {"ok": False, "connector": "salesforce", "error": "Could not get token"}
        except Exception as e:
            return {"ok": False, "connector": "salesforce", "error": str(e)}

    def push_contact(self, data: dict) -> dict:
        try:
            name = data.get("business_name","Unknown")
            exists = sf_lead_exists(name)
            if exists:
                return {"ok": True, "lead_id": exists, "action": "existing"}
            lead_id = sf_create_lead(
                name, data.get("website",""), data.get("phone",""),
                data.get("industry",""), data.get("location",""),
                data.get("key_insight","") or data.get("top_signal","")
            )
            return {"ok": bool(lead_id), "lead_id": lead_id, "action": "created"}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    def push_deal(self, contact_id: str, data: dict) -> dict:
        return {"ok": True, "note": "Salesforce uses Opportunities — create via CRM UI."}

    def push_note(self, contact_id: str, text: str) -> dict:
        try:
            sf_create_task(contact_id, text)
            return {"ok": True}
        except Exception as e:
            return {"ok": False, "error": str(e)}


# ── Airtable connector (env-based, framed as Intelligence DB) ─────────────

class AirtableConnector(CRMConnector):
    def test_connection(self) -> dict:
        if not (AIRTABLE_TOKEN and AIRTABLE_BASE_ID):
            return {"ok": False, "connector": "airtable", "error": "Not configured"}
        try:
            r = req.get(
                f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/{AIRTABLE_TABLE_ID}?maxRecords=1",
                headers={"Authorization": f"Bearer {AIRTABLE_TOKEN}"}, timeout=8
            )
            return {"ok": r.status_code == 200, "connector": "airtable"}
        except Exception as e:
            return {"ok": False, "connector": "airtable", "error": str(e)}
    ...
    def push_contact(self, data: dict) -> dict:
        fields = {
            ...
            "Score":            data.get("opportunity_score") or data.get("overall_score"),
            "Key Insight":      data.get("key_insight","") or data.get("top_signal",""),
            "Outreach Status":  "Not Started",
            "Source Stage":     "batch",
        }
        ...
        try:
            ...
            if r.status_code == 200:
                return {"ok": True, "record_id": r.json().get("id"), "action": "created"}
            return {"ok": False, "error": f"HTTP {r.status_code}: {r.text[:120]}"}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    def push_deal(self, contact_id: str, data: dict) -> dict:
        return {"ok": True, "note": "Airtable is an Intelligence DB — deals tracked as records."}

    def push_note(self, contact_id: str, text: str) -> dict:
        try:
            existing = airtable_get_record_id(contact_id)
            if existing:
                airtable_update_record(existing, {"Key Insight": text})
            return {"ok": True}
        except Exception as e:
            return {"ok": False, "error": str(e)}
...
_VISIBLE_CONNECTORS = [
    {
        ...
        "auth":        "api_key",
        "icon":        "🟠",
        "description": "Sync analyzed businesses as contacts, deals, and notes to your HubSpot account.",
        "key_note":    "Your API key is used only for this sync and is never stored by Yelhao.",
    },
    {
        "id":          "salesforce",
        "name":        "Salesforce",
        ...
        "auth":        "env",
        "icon":        "☁️",
        "description": "Push leads to Salesforce using backend environment credentials.",
        "key_note":    "Uses server-side credentials. No key required from you.",
    },
    {
        "id":          "airtable",
        "name":        "Airtable",
        "type":        "Intelligence DB",
        "auth":        "env",
        "icon":        "🟣",
        "description": "Save intelligence records to Airtable as your analysis layer and operational data store.",
        "key_note":    "Uses server-side credentials. No key required from you.",
    },
    {
        "id":          "csv",
        "name":        "CSV",
        "type":        "Universal export",
        "auth":        "none",
        "icon":        "📄",
        "description": "Export enriched records as a CSV. Works with any CRM that accepts imports.",
        "key_note":    "No account needed. Download and import to any CRM.",
    },
]

def _get_connector(name: str, api_key: str = "") -> CRMConnector | None:
    name = (name or "").lower()
    if name == "hubspot":   return HubSpotConnector(api_key or os.environ.get("HUBSPOT_TOKEN",""))
    if name == "salesforce":return SalesforceConnector()
    if name == "airtable":  return AirtableConnector()
    return None


# ── Batch worker ──────────────────────────────────────────────────────────

def _run_batch_item(item: dict, niche: str = "") -> dict:
    """Run fast signal scan on one contact."""
    name     = item.get("business_name") or item.get("name","")
    website  = item.get("website","")
    location = item.get("location","")
    industry = item.get("industry","") or niche

    result = {**item, "batch_status": "analyzing", "error": None}
    try:
        init_clients()
        # Use identity-led discovery if no website
        if not website and (name or location):
            sig = discover_business_signals(name, location, industry)
            website = sig.get("website","")
        if not website:
            result["batch_status"] = "no_website"
            result["key_insight"]  = "No website found — signal scan skipped."
            return result

        # Fast pipeline (not deep — label clearly on frontend)
        report = run_fast_pipeline(name, location, {website: extract_text(*safe_get(website) or ("",""))}, {}, "", "", [], "gpt_only")
        result.update({
            "batch_status":     "done",
        ...
        # Attach verdict
        v = generate_verdict(report, "outreach")
        result["verdict_emoji"] = v["emoji"]
        result["verdict_text"]  = v["text"]
    except Exception as e:
        result["batch_status"] = "error"
        result["error"]        = str(e)[:120]
    return result


def _batch_worker(batch_id: str, items: list, niche: str):
    """Background thread for batch analysis."""
    results = []
    for i, item in enumerate(items):
        BATCH_JOBS[batch_id]["progress"] = {"done": i, "total": len(items), "current": item.get("business_name","")}
        r = _run_batch_item(item, niche)
        results.append(r)
    BATCH_JOBS[batch_id].update({"status": "done", "results": results, "ts": time.time()})
    print(f"[Batch] {batch_id} complete — {len(results)} items", flush=True)
...
def list_connectors():
    """GET /connectors — return visible connectors and their server-side status."""
    if get_code_type(request) is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    sf_live = bool(os.environ.get("SF_CLIENT_ID") or os.environ.get("SALESFORCE_CLIENT_ID"))
    at_live = bool(AIRTABLE_TOKEN and AIRTABLE_BASE_ID)
    statuses = {"hubspot": "key_required", "salesforce": "live" if sf_live else "not_configured",
                "airtable": "live" if at_live else "not_configured", "csv": "live"}
    connectors = [{**c, "status": statuses.get(c["id"], "unknown")} for c in _VISIBLE_CONNECTORS]
    return jsonify({"ok": True, "connectors": connectors})


@app.route("/connectors/test", methods=["POST"])
def test_connector():
    """POST /connectors/test — verify a connector key before running batch."""
    if get_code_type(request) is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    body = request.get_json(force=True, silent=True) or {}
    name    = body.get("connector","")
    api_key = body.get("api_key","")
    conn = _get_connector(name, api_key)
    if not conn:
        return jsonify({"ok": False, "error": f"Unknown connector: {name}"}), 400
    result = conn.test_connection()
    return jsonify(result)


@app.route("/connectors/<name>/sync", methods=["POST"])
def sync_connector(name):
    """POST /connectors/<name>/sync — push a single analyzed record to a CRM."""
    if get_code_type(request) is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    body    = request.get_json(force=True, silent=True) or {}
    api_key = body.get("api_key","")
    ...
    conn = _get_connector(name, api_key)
    if name == "csv":
        return jsonify({"ok": False, "error": "Use /batch/<id>/export for CSV"}), 400
    if not conn:
        return jsonify({"ok": False, "error": f"Unknown connector: {name}"}), 400

    contact_r = conn.push_contact(data)
    if not contact_r.get("ok"):
        return jsonify(contact_r)

    contact_id = contact_r.get("contact_id") or contact_r.get("lead_id") or ""
    note_text  = "\n".join([
        "Yelhao Signal Scan",
        f"Score: {data.get('overall_score','')}",
        f"Key Insight: {data.get('key_insight','')}",
        f"Top Weakness: {(data.get('weaknesses') or [''])[0]}",
        f"Website: {data.get('website','')}",
    ])
    if contact_id:
        conn.push_note(contact_id, note_text)
        conn.push_deal(contact_id, data)

    return jsonify({
        "ok":         True,
        "connector":  name,
        "contact_id": contact_id,
        "action":     contact_r.get("action","synced"),
        "key_note":   "Your API key was used only for this request and is not stored by Yelhao.",
    })


@app.route("/batch/analyze", methods=["POST"])
def batch_analyze():
    """POST /batch/analyze — start batch signal scan on a list of contacts."""
    if get_code_type(request) is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    _cleanup_batch_jobs()
    body  = request.get_json(force=True, silent=True) or {}
    items = body.get("contacts") or body.get("items") or []
    niche = body.get("niche","")

    if not items:
        return jsonify({"ok": False, "error": "contacts list is required"}), 400
    if len(items) > 100:
        return jsonify({"ok": False, "error": "Max 100 contacts per batch"}), 400

    batch_id = str(uuid.uuid4())[:8]
    BATCH_JOBS[batch_id] = {"status": "running", "items": items, "results": [], "progress": {}, "ts": time.time()}

    t = _threading.Thread(target=_batch_worker, args=(batch_id, items, niche), daemon=True)
    t.start()
    return jsonify({"ok": True, "batch_id": batch_id, "count": len(items), "status": "running"})


@app.route("/batch/<batch_id>", methods=["GET"])
def batch_status(batch_id):
    """GET /batch/<id> — poll batch progress."""
    if get_code_type(request) is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
...
@app.route("/batch/<batch_id>/push_crm", methods=["POST"])
def batch_push_crm(batch_id):
    """POST /batch/<id>/push_crm — push batch results to a CRM."""
    if get_code_type(request) is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    job = BATCH_JOBS.get(batch_id)
    if not job or job["status"] != "done":
        return jsonify({"ok": False, "error": "Batch not done or not found"}), 404

    body      = request.get_json(force=True, silent=True) or {}
    connector = body.get("connector","")
    api_key   = body.get("api_key","")
    conn      = _get_connector(connector, api_key)
    if not conn:
        return jsonify({"ok": False, "error": f"Unknown connector: {connector}"}), 400

    pushed, failed = 0, 0
    for item in job.get("results",[]):
        if item.get("batch_status") != "done":
            continue
        r = conn.push_contact(item)
        if r.get("ok"):
            pushed += 1
            cid = r.get("contact_id") or r.get("lead_id","")
            if cid:
                conn.push_note(cid, item.get("key_insight","") or "")
        else:
            failed += 1
...
@app.route("/batch/csv/upload", methods=["POST"])
def batch_csv_upload():
    """
    POST /batch/csv/upload — upload a CSV, auto-detect columns, return preview.
    ...
    if not f:
        return jsonify({"ok": False, "error": "No file uploaded. Send multipart with field 'file'."}), 400

    try:
        text   = f.read().decode("utf-8-sig", errors="replace")
        reader = _csv.DictReader(_io.StringIO(text))
        rows   = [row for row in reader][:200]   # cap at 200 for safety
        cols   = reader.fieldnames or []
    except Exception as e:
        return jsonify({"ok": False, "error": f"CSV parse error: {str(e)}"}), 400

    # Auto-detect column mapping
    _FIELD_HINTS = {
        "business_name": ["business name","company","name","business","organization","account"],
...
@app.route("/batch/csv/start", methods=["POST"])
def batch_csv_start():
    """POST /batch/csv/start — confirm column mapping and start analysis."""
    if get_code_type(request) is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    body     = request.get_json(force=True, silent=True) or {}
    ...
    job = BATCH_JOBS.get(batch_id)
    if not job or job.get("status") != "pending_confirm":
        return jsonify({"ok": False, "error": "batch_id not found or not in pending state"}), 404

    raw    = job.get("raw_rows",[])
    m      = {**job.get("mapping",{}), **mapping}   # user overrides win
    items  = []
    for row in raw:
        item = {}
        for field, col in m.items():
            item[field] = (row.get(col) or "").strip()
        if not item.get("business_name"):
            # Try first column as fallback name
            first_val = next(iter(row.values()),"").strip()
            item["business_name"] = first_val
        items.append(item)

    BATCH_JOBS[batch_id]["status"]  = "running"
    BATCH_JOBS[batch_id]["items"]   = items
...
@app.route("/batch/<batch_id>/export", methods=["GET"])
def batch_export(batch_id):
    """GET /batch/<id>/export — download enriched results as CSV.
    Accepts token via X-Agent-Token header OR ?token= query param
    (needed for window.open / direct download links).
    """
    code_type = get_code_type(request)
    if code_type is None:
        # Fallback: check query param (for direct download links)
        qtoken = (request.args.get("token") or "").strip().lower()
        if qtoken in ALL_CODES:
            code_type = "master" if qtoken == MASTER_CODE else "guest"
    if code_type is None:
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    job = BATCH_JOBS.get(batch_id)
    if not job or job["status"] != "done":
    ...
    if not results:
        return jsonify({"ok": False, "error": "No results to export"}), 404

    out    = _io.StringIO()
    fields = ["business_name","website","location","industry","phone","email",
              "overall_score","key_insight","top_weakness","verdict_emoji","verdict_text","batch_status","error"]
    ...
    return Response(
        out.getvalue(),
        mimetype="text/csv",
        headers={"Content-Disposition": f"attachment; filename=yelhao_batch_{batch_id}.csv"}
    )
