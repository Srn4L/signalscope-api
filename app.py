...
def extract_enrichment(website_pages, social_links, website_url=""):
    ...
    # Extract instagram and tiktok from social_links param if passed
    # (these come from find_social_links called before extract_enrichment)
    for plat in ['instagram', 'tiktok', 'facebook', 'youtube', 'yelp']:
...
def push_to_salesforce(report, website_pages, social_links, website_url=""):
    ...
    Silent fail - never blocks the report.
    """
    if not (SALESFORCE_CLIENT_ID and SALESFORCE_CLIENT_SECRET and SALESFORCE_INSTANCE_URL):
        return
    ...
    # FIX 2: Use website_url passed in directly (not website_pages["_url"])
...
def push_to_airtable(report, enrichment, website_url="", location=""):
    ...
    business_name = report.get('company', '')
    if airtable_business_exists(business_name):
        print(f"  [!] Airtable: skipping duplicate - {business_name} already exists")
        return
...
def run_deep_job(job_id, business_name, location, website_pages, social_text,
    ...
    try:
        ...
        try:
            push_to_salesforce(report, website_pages, social_links or {}, website_url=website_url)
    ...
    def deep_work():
        try:
            ...
            run_deep_job(job_id, business_name, location, pages, local_social,
                        ...
                        manual_email=manual_email, website_url=website_url)
        except Exception as e:
            JOBS[job_id] = {"status": "error", "error": str(e), "ts": time.time()}
