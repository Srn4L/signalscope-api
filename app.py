...
def score_business_fit(industry, location, rating, reviews, followers=0, filters=None):
    """
    Score a business based on filter match (0-100).
    HIGHER score = BETTER fit for Yelhao's target.
    Businesses closer to filter thresholds score higher.
    """
    ...
    # Businesses WITH FEWER reviews than max = better fit (growth opportunity)
    if reviews < filters["max_reviews"]:
        # Scale: lower reviews = higher score bonus
        review_ratio = 1 - (reviews / filters["max_reviews"])
        score += int(30 * review_ratio)  # Up to +30 points
    else:
        # Over threshold = penalty but don't exclude
        overage = min((reviews - filters["max_reviews"]) / filters["max_reviews"], 2)
        score -= int(15 * overage)  # Up to -30 points
    
    # ── Follower count scoring ───────────────────────────────────────────
    if followers < filters["max_followers"]:
        follower_ratio = 1 - (followers / filters["max_followers"]) if filters["max_followers"] > 0 else 1
        score += int(15 * follower_ratio)  # Up to +15 points
    else:
        score -= 10  # Too many followers = already established
    
    # ── Rating scoring ───────────────────────────────────────────────────
    # Sweet spot: 3.8-4.5 (room to improve, not too bad)
    if 3.8 <= rating <= 4.5:
        score += 10
    elif rating > 4.5:
        score += 5   # Too good, less opportunity
    elif rating < 3.5:
        score -= 5   # Too poor, might be abandoned
    
    # ── Location modifiers ───────────────────────────────────────────────
    if 'manhattan' in location or 'midtown' in location:
        # Manhattan needs MORE reviews to count as growth opportunity
        if reviews < filters["manhattan_review_threshold"]:
            score += 10  # Genuinely new Manhattan business
        if rating >= filters["min_rating_manhattan"] and reviews >= filters["manhattan_review_threshold"]:
            score -= 15  # Very established Manhattan
...
def prospect():
    ...
    try:
        ...
        limit = 10  # Default if limit is not a valid integer
    
    # Extract and normalize prospect filters
    ...
    if niche_validation["quality_score"] < 50:
        print(f"  [i] Low-quality niche: {niche} (score: {niche_validation['quality_score']})")  # Info
    ...
    # Fetch MORE businesses than needed for variety and better ranking
    fetch_limit = min(limit * 4, 25)  # Get 4x more to rank from
    ...
    # Deduplicate and apply maturity filter
    deduped = []
    for biz in raw:
    ...
    # ── Score each business by filter fit, rank, and pick top N ──────────
    scored = []
    for biz in deduped:
        fit_score = score_business_fit(
            industry=niche,
            location=location,
            rating=biz.get('rating'),
            reviews=biz.get('review_count'),
            followers=0,  # Instagram check happens later
            filters=filters
        )
        scored.append((fit_score, biz))
    
    # Sort by fit score (highest first), add slight randomization for variety
    import random
    random.shuffle(scored)  # Shuffle first for variety among equal scores
    scored.sort(key=lambda x: x[0], reverse=True)
    
    results = [biz for score, biz in scored[:limit]]
    
    # Log scoring breakdown with business names
    if scored:
        print(f"  -> Ranked {len(scored)} businesses by filter fit:")
        for i, (score, biz) in enumerate(scored[:limit], 1):
            print(f"     #{i}: {biz['business_name']} - score {score}/100 (rating={biz.get('rating', 0)}, reviews={biz.get('review_count', 0)})")
    print(f"  -> Analyzing top {len(results)} businesses")
    sys.stdout.flush()
    analyzed = []
    for biz in results[:limit]:
        try:
            ...
            # Extract Google Places data from ranked business
            gp_rating  = float(biz.get('rating') or 0)
            gp_reviews = int(biz.get('review_count') or 0)
            
            if len(name) < 3 or "/" in name:
                print(f"  [i] Skipping bad name: {name}")  # Info
                continue
            
            print(f"  -> Analyzing: {name}")
            sys.stdout.flush()

            # Guard against placeholder/invalid URLs
            from urllib.parse import urlparse
            parsed = urlparse(site)
            bad_domains = {"google.com", "www.google.com", "bing.com", "www.bing.com"}
            if parsed.netloc in bad_domains or not site or len(site) < 10:
                print(f"  [i] Skipping {name} - invalid website URL")
                continue
            ...
            if not html:
                print(f"  [i] Skipping {name} - no HTML")  # Info
                continue
            ...
            for plat in ["instagram", "facebook", "yelp", "tiktok"]:
                if plat in social_links:
                    try:
                        ...
                        log_error("Social page scrape", e, platform=plat)
            ...
            for path in ["/book", "/booking", "/about", "/services", "/menu"]:
                try:
                    ...
                    log_error("Extra page scrape", e, path=path)
            ...
            try:
                ...
                log_error("GPT analysis", gpt_err, business_name=name)
                continue
            ...
            if len(homepage_text) < 200 and len(social_data) < 100:
                print(f"  [i] Skipping {name} - insufficient data for reliable diagnosis")  # Info
                continue
            ...
            try:
                ...
                log_error("Airtable prospect push", at_err, business_name=name)
