...
def is_established_business(industry, location, rating, reviews, followers=0, filters=None):
    """
    Returns True if the business is too established to be a Yelhao target.
    Filters are configurable per request - allows tuning without code changes.
    """
    ...
    # Handle None values defensively - explicit type casts
    rating = float(rating or 0)
    reviews = int(reviews or 0)
    followers = int(followers or 0)

    # Strong social presence = established
    if followers >= filters["max_followers"]:
        return True

    # Manhattan/established markets = stricter
    if 'manhattan' in location or 'midtown' in location:
        return rating >= filters["min_rating_manhattan"] and reviews >= filters["manhattan_review_threshold"]

    # High review count alone = established (regardless of rating)
    if reviews >= filters["max_reviews"]:
        return True

    # Default: high rating + moderate reviews
    return rating >= 4.7 and reviews >= 300
...
def prospect():
    ...
    for biz in results[:limit]:
        try:
            ...
            # Quick Instagram check to catch established businesses with strong social presence
            followers = int(quick_instagram_check(name, site) or 0)  # Ensure never None
            
            if is_established_business(niche, location, gp_rating, gp_reviews, followers, filters):
                if followers > 0:
                    print(f"  [i] Filtering: {name} - established business (rating={gp_rating}, reviews={gp_reviews}, followers={followers})")  # Info
                else:
                    print(f"  [i] Filtering: {name} - established business (rating={gp_rating}, reviews={gp_reviews})")  # Info
                continue
