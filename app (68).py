...
# ═══════════════════════════════════════════════════════════════════════════
# CONNECTOR ARCHITECTURE + BATCH QUEUE  (connectors_and_batch_v3)
# ═══════════════════════════════════════════════════════════════════════════
...
class CRMConnector(ABC):
    """
    Standard interface for all CRM connectors.

    user_key connectors (HubSpot):
        The user passes their own api_key at push time.
        It is used only for this request and never stored by Yelhao.

    env_var connectors (Salesforce, Airtable):
        api_key is ignored. Auth comes from Render env vars.
        Users configure these once at the account level, not per-request.
    """
    ...
    @abstractmethod
    def auth_model(self) -> str:
        """'user_key' or 'env_vars'"""
        ...

    @abstractmethod
    def test_connection(self) -> dict:
        """
        Returns {"ok": True,  "display_name": "Acme Corp"}
              | {"ok": False, "error": "Invalid token"}
        """
        ...

    @abstractmethod
    def push_contact(self, contact: dict) -> dict:
        """
        Upsert one contact/lead.
        contact keys: business_name, website, email, phone, location,
                      industry, score, headline, strengths, weaknesses
        Returns {"ok": True,  "id": "crm_id", "action": "created"|"updated"}
              | {"ok": False, "error": "..."}
        """
        ...

    def push_deal(self, contact_id: str, business_name: str, score: int) -> dict:
        return {"ok": True, "id": None}

    def push_note(self, contact_id: str, body: str) -> dict:
        return {"ok": True, "id": None}
...
#
# Writes standard HubSpot fields + optional Yelhao enrichment properties.
# If custom properties (yelhao_score etc.) don't exist in the user's portal,
# the push retries with standard fields only so the contact always gets created.
# ─────────────────────────────────────────────────────────────────────────────

_HS_CUSTOM_PROPS   = {"yelhao_score", "yelhao_headline", "yelhao_strengths", "yelhao_weaknesses"}
_HS_STANDARD_PROPS = {"company", "firstname", "website", "city", "email",
                      "phone", "lead_source", "hs_lead_status"}
...
class HubSpotConnector(CRMConnector):
    ...
    def name(self) -> str:       return "HubSpot"
    def auth_model(self) -> str: return "user_key"

    def test_connection(self) -> dict:
        try:
            r = req.get(f"{self.BASE}/crm/v3/objects/contacts?limit=1",
                        headers=self._h(), timeout=8)
            if r.status_code == 200:
                info = req.get(f"{self.BASE}/account-info/v3/details",
                               headers=self._h(), timeout=6)
                portal_name = (info.json().get("companyName", "")
                               if info.status_code == 200 else "")
                return {"ok": True,
                        "display_name": portal_name or "HubSpot Account"}
            return {"ok": False,
                    "error": f"HTTP {r.status_code} — check your Private App token"}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    def _find(self, email: str = "", website: str = "") -> str | None:
        """Email-first dedup, domain fallback."""
        try:
            if email:
                r = req.post(
                ...
                r = req.post(
                    ...
                    json={"filterGroups": [{"filters": [
                        {"propertyName": "website",
                         "operator": "CONTAINS_TOKEN", "value": domain}
                    ]}]})
                res = r.json().get("results", [])
                if res: return res[0]["id"]
        except Exception as e:
            log_error("HubSpotConnector._find", e)
        return None

    def _upsert(self, props: dict, existing_id: str = None) -> tuple[int, dict]:
        if existing_id:
            r = req.patch(
                f"{self.BASE}/crm/v3/objects/contacts/{existing_id}",
                headers=self._h(), json={"properties": props}, timeout=10)
        else:
            r = req.post(
    ...
    def push_contact(self, contact: dict) -> dict:
        ...
        if email:   full_props["email"] = email
        if contact.get("phone"): full_props["phone"] = contact["phone"]
        if score is not None:
            full_props["yelhao_score"]    = str(score)
        if headline:
            full_props["yelhao_headline"] = headline[:500]
        if contact.get("strengths"):
            full_props["yelhao_strengths"]  = _sl(contact["strengths"])
        if contact.get("weaknesses"):
            full_props["yelhao_weaknesses"] = _sl(contact["weaknesses"])

        existing_id = self._find(email, website)
        status, body = self._upsert(full_props, existing_id)

        # Custom props missing → retry with standard fields only
        if status == 400 and any(p in str(body) for p in _HS_CUSTOM_PROPS):
            safe = {k: v for k, v in full_props.items()
                    if k in _HS_STANDARD_PROPS}
            status, body = self._upsert(safe, existing_id)

        if status in (200, 201):
            return {"ok": True,
                    "id": existing_id or body.get("id"),
                    "action": "updated" if existing_id else "created"}
        return {"ok": False, "error": f"HTTP {status}: {str(body)[:200]}"}
...
# ─────────────────────────────────────────────────────────────────────────────
# SALESFORCE CONNECTOR  (env-var auth — wraps your existing sf_* functions)
# ─────────────────────────────────────────────────────────────────────────────

class SalesforceConnector(CRMConnector):

    def name(self) -> str:       return "Salesforce"
    def auth_model(self) -> str: return "env_vars"

    def test_connection(self) -> dict:
        token = sf_get_token()
        if token:
            return {"ok": True, "display_name": "Salesforce Account"}
        return {"ok": False,
                "error": ("Authentication failed — check SALESFORCE_CLIENT_ID, "
                          "CLIENT_SECRET, and INSTANCE_URL in Render env vars")}

    def push_contact(self, contact: dict) -> dict:
        name       = contact.get("business_name", "")
        score      = contact.get("score", 50) or 50
        headline   = contact.get("headline", "")
        weaknesses = contact.get("weaknesses", [])
        top_w      = weaknesses[0] if weaknesses else ""
        if isinstance(top_w, dict): top_w = top_w.get("text", str(top_w))

        existing = sf_lead_exists(contact.get("email", ""), name)
        if existing:
        ...
        lead_id = sf_create_lead(
            ...
            score           = score,
            primary_problem = top_w,
            key_insight     = headline,
            priority_score  = max(0, 100 - score),
        )
        if lead_id:
            return {"ok": True, "id": lead_id, "action": "created"}
        return {"ok": False,
                "error": "Salesforce lead creation failed — check server logs"}

    def push_note(self, contact_id: str, body: str) -> dict:
        task_id = sf_create_task(contact_id, "Yelhao Analysis", body)
        return {"ok": bool(task_id), "id": task_id}


# ─────────────────────────────────────────────────────────────────────────────
# AIRTABLE CONNECTOR  (env-var auth — Intelligence Database layer)
# Positioned as the operational data store / analysis layer, not a CRM peer.
# ─────────────────────────────────────────────────────────────────────────────

class AirtableConnector(CRMConnector):

    def name(self) -> str:       return "Airtable"
    def auth_model(self) -> str: return "env_vars"

    def test_connection(self) -> dict:
        if not (AIRTABLE_TOKEN and AIRTABLE_BASE_ID and AIRTABLE_TABLE_ID):
            return {"ok": False,
                    "error": ("Set AIRTABLE_TOKEN, AIRTABLE_BASE_ID, and "
                              "AIRTABLE_TABLE_ID in Render env vars")}
        try:
            r = req.get(
                f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}"
                f"/{AIRTABLE_TABLE_ID}?maxRecords=1",
                headers={"Authorization": f"Bearer {AIRTABLE_TOKEN}"},
                timeout=8)
            if r.status_code == 200:
                return {"ok": True, "display_name": "Airtable Intelligence Base"}
            return {"ok": False, "error": f"HTTP {r.status_code}"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
    ...
    def push_contact(self, contact: dict) -> dict:
        ...
        if score is not None:        fields["Signal Score"]      = score
        if headline:                 fields["Key Insight"]       = headline[:500]
        if contact.get("strengths"): fields["Strengths"]         = _sl(contact["strengths"])
        if contact.get("weaknesses"):fields["Weaknesses"]        = _sl(contact["weaknesses"])
        if contact.get("industry"):  fields["Industry"]          = contact["industry"]

        existing_rid = airtable_get_record_id(name)
        try:
            if existing_rid:
                ok = airtable_update_record(existing_rid, fields)
                return {"ok": ok, "id": existing_rid, "action": "updated"}
            r = req.post(
                f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}"
                f"/{AIRTABLE_TABLE_ID}",
                headers={"Authorization": f"Bearer {AIRTABLE_TOKEN}",
                ...
                timeout=10)
            if r.status_code == 200:
                return {"ok": True, "id": r.json().get("id"), "action": "created"}
...
# ─────────────────────────────────────────────────────────────────────────────
# HIDDEN CONNECTORS  (built, not surfaced in UI)
# ─────────────────────────────────────────────────────────────────────────────

class PipedriveConnector(CRMConnector):
    """Built but hidden — surface after testing against real accounts."""
    BASE = "https://api.pipedrive.com/v1"
    def _p(self): return {"api_token": self.api_key}
    def name(self) -> str:       return "Pipedrive"
    def auth_model(self) -> str: return "user_key"

    def test_connection(self) -> dict:
        try:
            r = req.get(f"{self.BASE}/users/me", params=self._p(), timeout=8)
            if r.status_code == 200:
                d = r.json().get("data", {})
                return {"ok": True, "display_name": d.get("company_name", "Pipedrive")}
            return {"ok": False, "error": f"HTTP {r.status_code}"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
    ...
    def push_contact(self, contact: dict) -> dict:
        ...
        try:
            r = req.post(f"{self.BASE}/persons", params=self._p(),
                         json=payload, timeout=10)
            if r.status_code == 201:
                return {"ok": True, "id": str(r.json()["data"]["id"]),
                        "action": "created"}
            return {"ok": False, "error": f"HTTP {r.status_code}"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
...
class GoHighLevelConnector(CRMConnector):
    """Built but hidden — endpoints need verification against real GHL account."""
    BASE = "https://rest.gohighlevel.com/v1"
    def _h(self): return {"Authorization": f"Bearer {self.api_key}",
                          "Content-Type": "application/json"}
    def name(self) -> str:       return "GoHighLevel"
    def auth_model(self) -> str: return "user_key"

    def test_connection(self) -> dict:
        try:
            r = req.get(f"{self.BASE}/contacts/", headers=self._h(),
                        params={"limit": 1}, timeout=8)
            return ({"ok": True, "display_name": "GoHighLevel Account"}
                    if r.status_code == 200
    ...
    def push_contact(self, contact: dict) -> dict:
        name  = contact.get("business_name", "")
        parts = name.split(" ", 1)
        payload = {"firstName": parts[0], "lastName": parts[1] if len(parts) > 1 else ".",
                   "companyName": name, "source": "Yelhao"}
        if contact.get("email"): payload["email"] = contact["email"]
        if contact.get("phone"): payload["phone"] = contact["phone"]
        try:
            r = req.post(f"{self.BASE}/contacts/", headers=self._h(),
                         json=payload, timeout=10)
            if r.status_code == 200:
                return {"ok": True,
                        "id": r.json().get("contact", {}).get("id"),
                        "action": "created"}
            return {"ok": False, "error": f"HTTP {r.status_code}"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
...
_ALL_CONNECTORS = {
    ...
    # Hidden — not surfaced in /connectors
    "pipedrive":   PipedriveConnector,
    "gohighlevel": GoHighLevelConnector,
    "ghl":         GoHighLevelConnector,
}

def get_connector(crm: str, api_key: str = "") -> CRMConnector | None:
    cls = _ALL_CONNECTORS.get((crm or "").lower().strip())
    return cls(api_key=api_key) if cls else None


# ─────────────────────────────────────────────────────────────────────────────
# CSV COLUMN DETECTION
# Auto-maps common export column names from popular CRMs to Yelhao fields.
# ─────────────────────────────────────────────────────────────────────────────

_CSV_FIELD_ALIASES = {
    "business_name": [
        "business name", "company", "company name", "organization",
        "account name", "name", "business", "firm", "client name",
    ],
    "website": [
        "website", "url", "web", "site", "domain", "website url",
        "company website", "web address",
    ],
    "location": [
        "location", "city", "city state", "city/state", "address",
        "full address", "mailing city", "billing city", "area",
    ],
    "email": [
    ...
    "phone": [
        ...
        "cell", "business phone", "office phone",
    ],
    "contact_id": [
        "id", "contact id", "record id", "crm id", "hubspot id",
        "salesforce id", "lead id", "account id",
    ],
}

def detect_csv_columns(headers: list[str]) -> dict:
    """
    Given CSV column headers, return a mapping of
    yelhao_field -> detected_csv_column.
    Undetected fields are mapped to None (user can override in the UI).
    """
    mapping   = {field: None for field in _CSV_FIELD_ALIASES}
...
def parse_csv_contacts(csv_text: str, column_mapping: dict) -> list[dict]:
    """
    Parse CSV text into a list of contact dicts using the confirmed mapping.
    Skips rows where business_name is empty after stripping.
    """
    contacts = []
    reader   = csv.DictReader(io.StringIO(csv_text))

    name_col = column_mapping.get("business_name")
    if not name_col:
        return contacts
    ...
    for row in reader:
        ...
        contacts.append({
            "contact_id":    (row.get(column_mapping.get("contact_id") or "") or "").strip(),
            "business_name": name,
            "website":       (row.get(column_mapping.get("website") or "") or "").strip(),
            "location":      (row.get(column_mapping.get("location") or "") or "").strip(),
            "email":         (row.get(column_mapping.get("email") or "") or "").strip(),
            "phone":         (row.get(column_mapping.get("phone") or "") or "").strip(),
        })

    return contacts


def build_enriched_csv(items: list[dict]) -> str:
    """
    Build an enriched CSV string from completed batch items.
    Includes original contact fields + all Yelhao analysis output.
    """
    output  = io.StringIO()
    columns = [
        # Original
        "business_name", "website", "location", "email", "phone", "contact_id",
        # Yelhao enrichment
        "signal_score", "score_confidence", "key_insight", "top_weakness",
        "top_strength", "pricing_position", "market_gap",
        "instagram", "facebook", "tiktok", "linkedin",
        "yelhao_status", "yelhao_error",
    ]
    ...
    for item in items:
        ...
        comp   = result.get("competitive_signals") or {}
        social = result.get("social_links") or {}

        weaknesses = result.get("weaknesses", [])
        strengths  = result.get("strengths", [])
        top_w = (weaknesses[0] if weaknesses else "")
        top_s = (strengths[0]  if strengths  else "")
        if isinstance(top_w, dict): top_w = top_w.get("text", "")
        if isinstance(top_s, dict): top_s = top_s.get("text", "")
        ...
        writer.writerow({
            ...
            "signal_score":     result.get("overall_score", ""),
            "score_confidence": result.get("score_confidence", ""),
            "key_insight":      (result.get("headline")
                                 or result.get("key_insight") or ""),
            "top_weakness":     top_w,
            "top_strength":     top_s,
            "pricing_position": comp.get("pricing_position", ""),
            "market_gap":       comp.get("market_gaps", ""),
            "instagram":        social.get("instagram", ""),
            "facebook":         social.get("facebook", ""),
            "tiktok":           social.get("tiktok", ""),
            "linkedin":         social.get("linkedin", ""),
            "yelhao_status":    item.get("status", ""),
            "yelhao_error":     item.get("error", ""),
        })
...
BATCH_JOBS      = {}
BATCH_TTL       = 3600
BATCH_DELAY     = 4.0    # seconds between items
BATCH_MAX_ITEMS = 100

# Temp store for CSV uploads awaiting column confirmation
# csv_id -> {"headers": [...], "mapping": {...}, "raw": "...", "preview": [...]}
CSV_UPLOADS = {}
CSV_UPLOAD_TTL = 600     # 10 minutes to confirm mapping
...
def cleanup_batch_jobs():
    ...
    stale = [k for k, v in BATCH_JOBS.items()
             ...
             and now - v.get("ts", now) > BATCH_TTL]
    for k in stale: del BATCH_JOBS[k]

    stale_csv = [k for k, v in CSV_UPLOADS.items()
                 if now - v.get("ts", now) > CSV_UPLOAD_TTL]
    for k in stale_csv: del CSV_UPLOADS[k]


def _scrape_socials(social_links: dict) -> dict:
    """
    Mirrors /agent social scraping — parallel fetch with OG metadata fallback.
    """
    if not social_links:
        return {}
    ...
    def _fetch(item):
        ...
        try:
            ...
            if h:
                ...
                meta = extract_social_metadata(h)
                if meta:
                    ms = " | ".join(f"{k}: {v}" for k, v in meta.items() if v)
                    return platform, f"[{platform.upper()} METADATA]: {ms}"
            return platform, f"[{platform.upper()} PRESENCE DETECTED]"
        except Exception as e:
            log_error("Batch _fetch_social", e, platform=platform)
            return platform, None

    with _TPE(max_workers=3) as ex:
        for platform, text in ex.map(_fetch, social_links.items()):
            if text:
                social_text[platform] = text
...
def _run_single_analysis(item: dict) -> dict:
    """
    Full signal analysis on one contact — same quality as /agent.
    Scrapes website + socials before calling run_fast_pipeline.
    """
    business_name = (item.get("business_name") or "").strip()
    website       = (item.get("website") or "").strip()
    location      = (item.get("location") or "").strip() or "United States"

    if not business_name:
        return {"error": "No business name provided"}
    ...
    if website:
        try:
            html, status = safe_get(website)
            if html and isinstance(status, int) and status < 400:
                website_pages["main"] = extract_text(html, 3000)
                social_links = find_social_links(html)
        except Exception as e:
            log_error("Batch website scrape", e, business=business_name)
    else:
        try:
            discovery = discover_business_signals(business_name, location)
            if discovery.get("website"):
                ...
                if html and isinstance(status, int) and status < 400:
                    website_pages["main"] = extract_text(html, 3000)
                    social_links = find_social_links(html)
            for k, v in (discovery.get("social_links") or {}).items():
                if k not in social_links:
                    social_links[k] = v
        except Exception as e:
            log_error("Batch discovery", e, business=business_name)

    social_text = _scrape_socials(social_links)

    try:
        init_clients()
        result = run_fast_pipeline(
            business_name, location, website_pages, social_text, social_links)
        result["company"]  = business_name
        result["website"]  = website
        result["location"] = location
        return result
    except Exception as e:
        log_error("Batch fast_pipeline", e, business=business_name)
        return {"error": str(e), "company": business_name}
...
def _batch_worker(batch_id: str):
    ...
    for i, item in enumerate(job["items"]):
        ...
        print(f"[Batch {batch_id}] {i+1}/{job['total']}: "
              f"{item['business_name']}", flush=True)
        try:
            result = _run_single_analysis(item)
            if result.get("error"):
                job["items"][i]["status"] = "failed"
                job["items"][i]["error"]  = result["error"]
                job["failed"] += 1
            else:
                job["items"][i]["status"] = "done"
                job["items"][i]["result"] = result
                job["done"] += 1
        except Exception as e:
            log_error("Batch worker", e, index=i)
            job["items"][i]["status"] = "failed"
    ...
    print(f"[Batch {batch_id}] Done — "
          f"{job['done']} ok, {job['failed']} failed", flush=True)


def _make_batch_job(valid_items: list) -> tuple[str, dict]:
    ...
    threading.Thread(target=_batch_worker, args=(batch_id,), daemon=True).start()
    return batch_id, job
...
def list_connectors():
    """
    Returns the visible connectors and their auth model.
    Pipedrive and GoHighLevel are intentionally excluded until tested.
    """
    return jsonify({
        "connectors": [
            {
                ...
                "name":        "HubSpot",
                "role":        "CRM Sync",
                "description": "Push analyzed leads directly into your HubSpot pipeline.",
                "status":      "live",
                "auth":        "user_key",
                "key_label":   "Private App Token",
                "key_help":    "HubSpot Settings → Integrations → Private Apps → Create app",
                "stores_key":  False,
            },
            {
                "id":          "salesforce",
                "name":        "Salesforce",
                "role":        "CRM Sync",
                "description": "Push analyzed leads into Salesforce as Lead records with tasks.",
                "status":      "live",
                "auth":        "env_vars",
                "key_label":   None,
                "key_help":    ("Requires SALESFORCE_CLIENT_ID, CLIENT_SECRET, and "
                                "INSTANCE_URL set in Render env vars by the account owner."),
                "stores_key":  None,
            },
            {
                "id":          "airtable",
                ...
                "description": ("Store Yelhao's signal data in Airtable as your "
                                "operational intelligence layer. Use it to track, "
                                "filter, and prioritize before pushing to your CRM."),
                "status":      "live",
                "auth":        "env_vars",
                "key_label":   None,
                "key_help":    ("Requires AIRTABLE_TOKEN, AIRTABLE_BASE_ID, and "
                                "AIRTABLE_TABLE_ID set in Render env vars."),
                "stores_key":  None,
            },
            {
                "id":          "csv",
                ...
                "role":        "Universal",
                "description": ("Works with any CRM that exports contacts as CSV — "
                                "Pipedrive, Zoho, GoHighLevel, Monday, Notion, and more. "
                                "Upload your export, run analysis, download the enriched file."),
                "status":      "live",
                "auth":        "none",
                "key_label":   None,
                "key_help":    "No API key needed. Upload a CSV file to get started.",
                "stores_key":  False,
            },
        ]
    })
...
def test_connector_route():
    """
    Verify CRM credentials before starting a batch.
    Key is tested immediately and never stored.

    POST: { "crm": "hubspot", "api_key": "pat_xxxx" }
    Response: { "ok": true, "display_name": "Acme Corp" }
    """
    if get_code_type(request) is None:
        return jsonify({"error": "Access denied."}), 401

    body    = request.get_json(silent=True) or {}
    crm     = (body.get("crm") or "").lower().strip()
    api_key = (body.get("api_key") or "").strip()

    connector = get_connector(crm, api_key)
    if not connector:
        return jsonify({"ok": False,
                        "error": f"'{crm}' is not supported"}), 400
...
def batch_analyze():
    """
    Start batch analysis from a JSON list of contacts.
    ...
    Response 202:
    { "batch_id": "abc123", "total": 12, "est_minutes": 4.5,
      "poll_url": "/batch/abc123" }
    """
    if get_code_type(request) is None:
        return jsonify({"error": "Access denied."}), 401
    ...
    if len(contacts) > BATCH_MAX_ITEMS:
        return jsonify({"error": f"Max {BATCH_MAX_ITEMS} per batch. "
                                 f"You sent {len(contacts)}."}), 400
    ...
    for c in contacts:
        ...
        valid.append({
            "contact_id":    (c.get("contact_id") or "").strip(),
            "business_name": name,
    ...
    if not valid:
        return jsonify({"error": "No valid contacts — each needs a business_name"}), 400

    batch_id, job = _make_batch_job(valid)
    est_minutes   = round(len(valid) * (BATCH_DELAY + 15) / 60, 1)

    print(f"[Batch {batch_id}] Started — {len(valid)} contacts, "
          f"est {est_minutes}m", flush=True)

    return jsonify({
        "batch_id":    batch_id,
        "total":       len(valid),
        "est_minutes": est_minutes,
        "message":     f"Analysis started. Poll /batch/{batch_id} for progress.",
...
def batch_status(batch_id):
    """
    Poll live batch progress.
    Results appear per-item as they complete — no need to wait for the full batch.
    """
    if get_code_type(request) is None:
        return jsonify({"error": "Access denied."}), 401
...
def batch_push_crm(batch_id):
    """
    Push completed analysis results to the user's CRM.

    ── Key policy ───────────────────────────────────────────────────────────
    HubSpot:               user supplies api_key in this request only.
                           Key is never stored or logged by Yelhao.
    Salesforce / Airtable: no api_key needed — uses Render env vars.
    ─────────────────────────────────────────────────────────────────────────

    POST body:
    {
      "crm":        "hubspot" | "salesforce" | "airtable",
      "api_key":    "required for hubspot only",
      "push_deals": true,
      "push_notes": true
    }
    """
    ...
    crm        = (body.get("crm") or "").lower().strip()
    api_key    = (body.get("api_key") or "").strip()
    push_deals = body.get("push_deals", True)
    push_notes = body.get("push_notes", True)

    if not crm:
        return jsonify({"error": "crm is required (hubspot, salesforce, airtable)"}), 400

    if crm == "hubspot" and not api_key:
        return jsonify({
            "error": ("HubSpot requires an api_key. "
                      "Your key is used only for this request — "
                      "Yelhao never stores it.")
        }), 400

    connector = get_connector(crm, api_key)
    if not connector:
        return jsonify({"error": f"'{crm}' is not a supported connector. "
                                 "Use: hubspot, salesforce, airtable"}), 400

    completed = [item for item in job["items"]
                 if item["status"] == "done" and item.get("result")]
    if not completed:
        return jsonify({
            "error": ("No completed analyses to push yet. "
                      "Check /batch/<id> for progress.")
        }), 400
    ...
    for item in completed:
        ...
        score      = (result.get("overall_score")
                      or result.get("opportunity_score") or 0)
        weaknesses = result.get("weaknesses", [])
        strengths  = result.get("strengths", [])
        headline   = (result.get("headline")
                      or result.get("key_insight")
                      or result.get("top_recommendation") or "")

        contact_payload = {
            "business_name": name,
            "website":       item.get("website") or result.get("website") or "",
            "email":         item.get("email") or result.get("email") or "",
            "phone":         item.get("phone") or result.get("phone") or "",
            "location":      item.get("location") or result.get("location") or "",
        ...
        cr = connector.push_contact(contact_payload)
        if not cr.get("ok"):
            failed += 1
        ...
        crm_id = cr.get("id")
        if cr.get("action") == "updated": updated += 1
        else:                             pushed  += 1

        if push_deals and crm_id:
            connector.push_deal(crm_id, name, score)

        if push_notes and crm_id:
            comp = result.get("competitive_signals") or {}
            note = [f"[Yelhao] Signal Analysis — {name}",
                    f"Score: {score}/100", ""]
            if headline:
                note += [f"Key Insight: {headline}", ""]
            if weaknesses:
                w = (weaknesses[0] if isinstance(weaknesses[0], str)
                     else weaknesses[0].get("text", str(weaknesses[0])))
                note += [f"Top Weakness: {w}", ""]
            if comp.get("pricing_position"):
                note.append(f"Pricing: {comp['pricing_position']}")
            if comp.get("market_gaps"):
                note.append(f"Market Gap: {comp['market_gaps']}")
            note += ["", "Generated by Yelhao · yelhoa.netlify.app"]
            connector.push_note(crm_id, "\n".join(note))
    ...
    return jsonify({
        ...
        "total":    len(completed),
        "errors":   errors,
        "key_note": ("Your API key was used only for this request "
                     "and is not stored by Yelhao."),
    })
...
def csv_upload():
    """
    Step 1 of CSV import: upload a file, get back detected column mapping + preview.
    ...
    Response:
    {
      ...
      "row_count": 47,
      "message": "Column mapping auto-detected. Confirm or adjust before starting analysis."
    }
    """
    if get_code_type(request) is None:
        return jsonify({"error": "Access denied."}), 401
    ...
    try:
        ...
        try:
            ...
            return jsonify({"error": f"Could not decode CSV: {str(e)}"}), 400

    reader  = csv.DictReader(io.StringIO(raw))
    headers = reader.fieldnames or []

    if not headers:
        return jsonify({"error": "CSV has no headers — first row must be column names"}), 400

    mapping   = detect_csv_columns(list(headers))
    rows      = list(reader)
    row_count = len(rows)
    preview   = [dict(r) for r in rows[:5]]

    if len(CSV_UPLOADS) > 100:
        cleanup_batch_jobs()

    csv_id = str(uuid.uuid4())[:8]
    CSV_UPLOADS[csv_id] = {
        "raw":       raw,
    ...
    return jsonify({
        ...
        "preview":   preview,
        "row_count": row_count,
        "message":   ("Column mapping auto-detected. "
                      "Confirm or adjust in the UI, then POST to /batch/csv/start."),
    })
...
def csv_start():
    """
    Step 2: confirm column mapping and start batch analysis.
    ...
    Response 202: same as /batch/analyze
    """
    if get_code_type(request) is None:
        return jsonify({"error": "Access denied."}), 401
    ...
    upload = CSV_UPLOADS.get(csv_id)
    if not upload:
        return jsonify({"error": "csv_id not found or expired. "
                                 "Re-upload the file."}), 404

    if not mapping.get("business_name"):
        return jsonify({"error": "mapping must include a business_name column"}), 400

    contacts = parse_csv_contacts(upload["raw"], mapping)

    if not contacts:
        return jsonify({"error": "No valid rows found after applying mapping. "
                                 "Check that business_name column is correct."}), 400

    if len(contacts) > BATCH_MAX_ITEMS:
        return jsonify({"error": f"Max {BATCH_MAX_ITEMS} rows per batch. "
                                 f"Your CSV has {len(contacts)} valid rows."}), 400
    ...
    # Clean up the upload now that we've consumed it
    del CSV_UPLOADS[csv_id]

    batch_id, _ = _make_batch_job(valid)
    est_minutes = round(len(valid) * (BATCH_DELAY + 15) / 60, 1)

    print(f"[Batch {batch_id}] CSV import — {len(valid)} rows, "
          f"est {est_minutes}m", flush=True)
    ...
    return jsonify({
        ...
        "total":       len(valid),
        "est_minutes": est_minutes,
        "message":     (f"Analysis started for {len(valid)} contacts from CSV. "
                        f"Poll /batch/{batch_id} for live progress. "
                        f"Download enriched results at /batch/{batch_id}/export "
                        f"when done."),
        "poll_url":    f"/batch/{batch_id}",
        "export_url":  f"/batch/{batch_id}/export",
    }), 202
...
def batch_export_csv(batch_id):
    """
    Download completed batch results as an enriched CSV.
    Includes all original contact fields + Yelhao signal analysis.
    Works for both JSON and CSV-imported batches.
    ...
    if job["status"] == "running" and job["done"] == 0:
        return jsonify({
            "error":   "Analysis not started yet. Check progress at /batch/<id>.",
            "percent": 0,
        }), 202

    csv_content = build_enriched_csv(job["items"])
    ...
    return Response(
        ...
        mimetype="text/csv",
        headers={
            "Content-Disposition": f'attachment; filename="yelhao_enriched_{batch_id}.csv"',
            "X-Batch-Status":      job["status"],
            "X-Batch-Done":        str(job["done"]),
            "X-Batch-Total":       str(job["total"]),
        }
    )
