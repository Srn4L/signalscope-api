...
def calculate_score(report):
    ...
    # Sub-scores from GPT if available
    sub = report.get("scores", {})
    if sub:
        vals = [v.get("score", 0) for v in sub.values() if isinstance(v, dict) and v.get("score")]
        if vals:
            avg_sub = sum(vals) / len(vals)
            # Blend: 60% text/rating signals, 40% GPT sub-scores
            score = round(score * 0.6 + avg_sub * 0.4)
